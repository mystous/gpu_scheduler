...
## 1.6 Philly-2K-C48 임계부하 실측 (2026-06-05~06 밤) — 표준 워크로드 첫 캠페인

설계: 층화 2000(클램프 48h — GPU 구성 원본 87/2/5/6%, 8-GPU가 GPU-시간 60%),
**평균 부하 1.01×(임계) + peak 8.9×**, S=1600(바닥 82% — 상위 18% 비율 보존), run당 ~2.7~3.2h.

| 정책 | 큐잉 p50/p90/p99/max | BSLD p50/p90/max | 완료 |
|---|---|---|---|
| Kueue | 1838 / 4943 / 5130 / 5963 | 166 / 455 / 597 | 2000 |
| EASY(완벽추정†) | 1201 / 2657 / 3068 / **3309** | 113 / 265 / **331** | 2000 |
| **sfqa-auto(τ=10)** | **340 / 1167** / 3490 / 4874 | **32 / 97** / 467 | 2000 |

- **auto가 분포의 ~97%를 지배**: p50은 EASY의 1/3.5·Kueue의 1/5.4, p90도 −56%/−76%.
  꼬리 곡선은 p97 부근에서 교차 — 상위 3% 극단만 EASY 우위.
- **부하 영역 3종 스토리 완성**: 저부하(S=360) Kueue 압승 → **임계(C48) auto 몸통 지배·EASY 꼬리 방어**
  → 과부하(κ6000)에서도 auto가 균형 유지. "경합이 깊을수록 보호 정책의 가치, auto는 그중 균형점".
- †EASY 캐비앗 2개: 완벽 duration 추정 가정 + 이번 조건(5s-churn 82%)에선 예약 닻이 약해
  multi-GPU 선두가 backfill에 추월당하는 현상 관측(03시 디버그) — max 해석 주의.
- 그래프: `c48_interim.png`(시간축 추이·백분위·꼬리), 재현: `chain_c48.sh`.
