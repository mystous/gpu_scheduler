...
|  `philly_2k_c48.csv` (**Philly-2K-C48**) | **층화 2000, JCT>2일은 2일로 절단(클램프)** — 잡 제거 없음 → GPU 잡수 분포 원본 그대로(87/2/5/6%), GPU-시간 비중 8-GPU 60%, dur p50 22.2분/max 48h(절단 65건). drop 방식은 multi-GPU 장기 잡을 빼서 구성을 왜곡하므로 폐기(사용자 결정 2026-06-05) |

명명 규칙: `<트레이스>-<샘플수>-C<절단h>` (C=클램프, D=드롭). 예: **Philly-2K-C48** = 층화 2000,
duration 48h 절단. 차기 실험의 표준 워크로드.
