...
\caption{Helios(Venus) 과부하 이종: $q_{p50}$(HOL 대기) 대 \texttt{lt50\%}(추월 순서 불공정) 트레이드오프. 본문 \S\ref{eval:robust:trace}.}
\label{fig:helios}
\end{figure}
...
무튜닝 SAFA는 FIFO 수준의 순서 공정성을 유지(Philly 기준 하위 1\% 50--76 수준)하면서 중앙값 대기를 FIFO 대비 50--61\% 단축하여, 공정성을 포기하지 않고 빨라졌다. 단일·저경합에서는 Lucid 같은 collocation 정책이 더 빠르고 공정할 수 있으나, 이질성·부하가 깊어질수록 다른 빠른 정책들이 아사(공정성 붕괴)를 겪는 반면 SAFA는 큐 정보만으로 그 아사를 예방했고, 독립 트레이스(Helios)에서도 큰 작업이 추월당해 순서 공정성을 잃는 비율을 정보-우위 그리디의 약 1/3로 억제했다. 즉 SAFA는 별도 프로파일이나 부차 정보 없이 큐에서 관측되는 정보만으로, 코어 스케줄러를 그대로 둔 채 그 위에서 아사를 예방하면서 공정성을 유지하는 메타 스케줄러다.
