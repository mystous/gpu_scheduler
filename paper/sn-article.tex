...
SAFA가 ``배치-전 어드미션 계층''이라는 주장의 운영적 실체를 위해, 배치 스케줄러를 교체하지 않고 SAFA를 끼워 넣는 통합을 구체화한다. 쿠버네티스에서는 커스텀 스케줄러 빌드 없이 \texttt{PodSchedulingGates}로 실현한다: 각 Pod에 게이트 \texttt{safa.io/admission}를 부여해 기본 \texttt{kube-scheduler} 후보에서 잠시 빼고, SAFA 컨트롤러가 주기 $\Delta$마다 게이트된 Pod의 대기 나이·자원 요청을 관측해 식 \eqref{eq:auto-pstar}로 $P^*$를 계산한 뒤 가장 높은 Pod부터 게이트를 해제한다(Algorithm \ref{alg:k8s}). 게이트가 풀린 Pod만 기본 스케줄러가 그 순서로 binding하므로 배치 로직은 전혀 손대지 않는다 --- Kueue와 결합하면 LocalQueue 제출 직전, 커스텀 플러그인과 결합하면 QueueSort 앞의 전처리로 동작한다. 운영상 가장 중요한 점은 \textbf{게이트 해제 규율이 공정성 축을 가른다}는 것이다: 선두가 막혔을 때 후속을 계속 푸는 greedy 비차단 해제는 중앙 대기를 7.5배 줄이는 대신 worst-1\% p1을 0으로 붕괴시키므로, 시뮬과 같은 p1 보존을 원하면 \textbf{선두 fit 불가 시 그 패스의 해제를 중단}(blocking-aware)해야 한다 --- 이 대비는 \S\ref{eval:b200}의 버스트 도착 B200 \textit{실측}으로 확인된다 --- blocking$+$counter는 p1 82.8, 기존 greedy는 3.8이다(단 규율 단독으로는 부족하다 --- blocking을 SJF·Themis 정렬에 줘도 p1 0이라, p1 유지는 도착순 골격$+$blocking$+$제한적 승급의 합작이다). 권장 구현(blocking-aware 해제, 도착-순번 나이)은 컨트롤러 참조 구현으로 제공한다. 그 밖의 운영 요건 --- 멀티노드 gang의 노드 단위 fit 판정·잠정 점유 계상, 영구 fit 불가 작업의 wedge dead-letter 격리, 멀티테넌트에서 VC 내부 적용(VC 간 쿼타는 Kueue에 위임), Pod-수준 게이트의 etcd 부하를 피하는 Kueue suspend 결합, fail-open과 전략 내성 --- 은 \textit{멀티노드·멀티테넌트} 배포의 설계 사항이며, 본 참조 구현은 단일 노드(B200$\times$8)를 대상으로 집계 용량으로 단순화했으므로(단일 노드에서 노드 fit과 등가) 이들의 구현·검증은 향후 과제로 둔다(\S\ref{eval:threats}).
