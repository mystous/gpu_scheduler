...
GPU 클러스터에서 학습·추론 작업을 어떻게 스케줄링하느냐는 시스템 최적화의 핵심 문제다. GPU 할당률과 사용률이 곧 운영 비용과 연구 비용을 좌우하기 때문이다. 게다가 GPU 자원은 어느 조직에서나 부족하기 마련이어서, 더 나은 스케줄링을 찾으려는 연구가 오랫동안 이어져 왔다. 규모가 큰 일부 조직은 고대역폭 인터커넥트로 수백~수천 장의 GPU를 묶은 전용 ``하이퍼클러스터''를 구성하지만, 이는 비용을 키우고 확장성을 제약할 뿐 아니라 클러스터 사이에 자원 단편화(fragmentation)를 낳는다\cite{athlur2022varuna}. 프로덕션 트레이스 분석도 같은 문제를 가리킨다. 공유 환경에서 부분(fractional) GPU를 할당하면 단편화가 심해져, 수백 장의 GPU가 유휴 상태로 남은 채 새 작업에 배정되지 못한다\cite{weng2023beware}. 결국 모델이 커지고 워크로드가 다양해질수록 GPU 자원을 효율적으로 관리하는 스케줄링이 더욱 중요해진다. 그러나 모든 상황에 통하는 해법은 없다. 완료 시간을 알 수 없는 작업을 큐에 배정하는 문제 자체가 NP-hard 조합 최적화이고\cite{ullman1975np}, 실제 스케줄링은 flow shop·job shop·단일 기계·open shop·$k$-기계 병렬 스케줄링이 뒤섞인 형태라\cite{b1} 대개 근사 알고리즘에 기댈 수밖에 없기 때문이다\cite{b2}.

이런 어려움에도 딥러닝 작업을 위한 스케줄러 연구는 꾸준히 발전해 왔다. Ye 등\cite{b3}은 이 분야를 두 축으로 정리했다. 하나는 워크로드 유형(학습·추론 등)에 따른 분류이고, 다른 하나는 효율성·공정성·마감 기한 같은 목표에 따른 분류다. 이들은 사용률·공정성·완료 시간에서 두각을 보인 Sia\cite{b4}와 다중 상태 프로파일링으로 성능을 끌어올린 Lucid\cite{b5}를 대표적인 최신(state-of-the-art) 스케줄러로 꼽았으며, 오늘날 GPU 스케줄러 대부분이 이런 접근의 변형이다. 다만 이들은 한 가지 부담을 공유한다. 워크로드 특성이 바뀌면 정책이나 스케줄러 자체를 다시 맞춰야 한다는 점이다.

이렇게 최근 제안된 GPU 스케줄러들도 저마다 한계를 안고 있다. 예컨대 learning-to-rank 기반 LLM 스케줄러에서는 순위 지표인 Kendall's Tau가 종단 간(end-to-end) 성능을 제대로 반영하지 못하는 경우가 보고됐다. 가장 짧은 70\% 요청의 내부 순서를 무작위로 섞어도 Tau는 0.5로 유지되지만 지연 시간은 크게 늘어난다(ShareGPT의 Llama-8B 기준 1.8배). 게다가 이 방법은 chunk-prefill이나 prefill-decode 분리(disaggregation) 같은 최신 LLM 서빙 최적화와의 결합이 아직 충분히 검증되지 않았다\cite{fu2024efficient}. GPU 공유 워크로드를 겨냥한 Fragmentation Gradient Descent(FGD)\cite{weng2023beware} 같은 단편화 인지 스케줄러도 마찬가지다. 스케줄러 독립성을 지키려고 단편화를 ``한 스텝(one step)''이라는 좁은 범위로만 측정하는데, 이 좁은 시야는 자원이 넉넉하고 단편화가 낮은 초기 단계에서 오히려 차선의(suboptimal) 결과를 낳을 수 있다.

자원 할당을 정교하게 최적화하는 이런 스케줄러일수록 복잡성과 한계도 크다. 예컨대 Optimus\cite{peng2018optimus} 같은 동적 스케줄러는 온라인 성능 모델로 학습 속도와 수렴을 추정하는데, 효과적이긴 하나 예측 오차에 민감하다. 특히 수렴 추정에서 최대 20\%의 오차가 관찰됐다. 또 자원 구성을 바꿀 때마다 체크포인트에 의존하므로 오버헤드가 생기며, 워커나 파라미터 서버가 수백 개에 이르는 작업에서는 그 부담이 상당하다. 한편 Themis\cite{mahajan2020themis}는 공정성과 배치 민감성(placement sensitivity)을 노리지만, 그 대가로 구조와 계산이 크게 복잡해진다. Themis는 앱이 제출한 입찰가(bid)를 바탕으로 중앙 ARBITER가 라운드마다 자원을 경매하는데, 부분 할당 경매에 솔버(예: Gurobi)를 쓰기 때문에 계산 오버헤드가 크다(경합이 심하면 95퍼센타일 1,398ms). 더구나 그 효과는 앱이 제출하는 복잡한 입력 --- 비이상적 배치로 인한 속도 저하까지 정밀하게 모델링한 완료 시간 공정성(finish-time fairness) 추정값 --- 에 의존한다.
...
이러한 단편화에서 비롯되는 HOL 블로킹\cite{karol1987hol}(선두 작업이 막혀 뒤따르는 작업까지 지연되는, 패킷 스위칭에서 비롯된 고전 현상)은 GPU 서버 풀 운영에서 구조적으로 발생하며(\S\ref{algo:intro}), 백필을 허용하는 스케줄러에서도 큰 작업이 선두에서 막혀 후속 작업의 배치를 지연시킨다. 이는 배치 정책과 독립적인, 어드미션 순서 단계에서의 개입 필요성을 뒷받침한다.
...
다만 SAFA는 큐 순서만을 조정하므로, 백로그가 얕은 일부 상황에서는 기아 작업을 상위로 승급시켜도 작은 작업에 자리를 추월당할 수 있다. 이는 향후 연구에서 감지(부하·나이 기반 기아 압력)와 자원 예약(EASY식 자리 잠금, 단 duration 추정 불요)을 결합하여 해소할 계획이다 --- 다만 \textit{예약}(자리 잠금)과 \textit{재정렬}(나이 승급)은 같은 큐에서 충돌할 수 있으므로, 그 상호작용을 분석해 두 메커니즘을 정합시키는 것이 결합의 선결 과제다. 또한 본 평가는 GPU 갱 트레이스(Philly·사내 K8S)에 한정되므로, 표준 워크로드 포맷(SWF) 기반 고전 병렬 작업 트레이스로 평가를 확장해 고전 HPC 배치 스케줄링 문헌과의 가교를 놓는 것도 향후 과제다. 그 밖의 가장 중요한 향후 과제는 \textit{다중 노드(2노드 이상)} 실 클러스터 실측이다: (i) 노드 간 단편화가 실재하는 레짐에서의 효율 우위 실측(단일 노드에는 그 레짐 자체가 형성되지 않는다), (ii) 멀티노드 gang의 부분 바인딩·원자성 검증, (iii) 노드 간 이주·네트워크 비용 계측, (iv) Kueue-suspend 결합의 per-VC 공정성 정량화.
