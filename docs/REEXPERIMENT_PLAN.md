...
```
┌─ 실제 K8S 레이어 (신규) ─────────────────────────────────────────┐
│  ① SFQA = kube-scheduler 플러그인 (QueueSort 확장점)              │
...
│   gang 스케줄링: kube-scheduler-plugins Coscheduling 플러그인     │
│  하드웨어: H100 ×3 node, B200 ×1 node                            │
...
| 학습/임의 | NVIDIA `cuda-checkpoint` + CRIU (H100/B200 GPU 프로세스 C/R) | 투명 C/R 시간 (PipeSwitch/Singularity 반박 근거) |
| 추론(vLLM) | drain → kill → 타겟 노드 재기동 | 모델 weight 재적재(지배적) + warmup |

## 4. 단계별 로드맵

- **Phase 0 — 기반**: K8S 클러스터 라벨링(H100/B200), Kueue+DCGM+Prometheus 설치, 트레이스→K8S Job 변환기, 메트릭 수집 파이프라인.
- **Phase 1 — SFQA 실측 (필수)**: P* 우선순위 컨트롤러 → Kueue 반영. baseline(FIFO/우선순위/Kueue 기본) 대비 JCT·할당률. **저위험·확실한 실제 결과.**
