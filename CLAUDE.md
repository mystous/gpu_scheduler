...
- **K8S 실측 캠페인**: `results/` (SUMMARY.md, runs_summary.csv, overheads/). 원본 run 데이터는 실험 서버 `/raid/squad/` (저장소 외부).
- **Python 시뮬 부하 스윕**: `sim/sweep_results/` (FINDINGS.md, sweep_table.csv, 그래프, `*_raw.tar.gz`).
- **raw 압축 해제본**: `sim/sweep_results/raw/<실험명>/` — 분석 편의를 위해 tar.gz를 풀어둔 것으로 **gitignore 대상**(약 830MB, 커밋 금지). 없으면 같은 디렉토리의 `*_raw.tar.gz`에서 재추출: `for f in *_raw.tar.gz; do mkdir -p raw/${f%_raw.tar.gz}; tar xzf $f -C raw/${f%_raw.tar.gz}; done`. 내부: 정책별 `<policy>_jobs.csv`(잡별 큐잉·서비스 시간), `<policy>_alloc.csv`(할당률 시계열).
...
6줄(필수)로 하이퍼파라미터 그리드 스윕을 정의한다(`linux_main.cpp::parse_config_file`에서 파싱):
...
<r_penalty>                                # 선택(7행): R 감쇠 (기본 0.1)
<priority_base>                            # 선택(8행): P = 1/base^j 의 밑 (기본 2.0)
<queue_prefix_mult>                        # 선택(9행): SFQA 재정렬 창 = 서버수×배수 (기본 3)
```

7~9행은 민감도 스윕용 선택 항목으로, 생략하면 `global_const` 기본값을 쓴다.
...
3. **`job_scheduler`**(추상 베이스) — 단일 배치 결정. 서브클래스가 `arrange_server(job, queue_index, coprocessor)`를 구현한다:
   ...
   - `scheduler_fare_share` — 페어 셰어 균형 (**미구현** — `return 0`만 반환)
...
상세 클래스 멤버/메서드는 `docs/CLASS_REFERENCE.md`, 알고리즘 설명은 `docs/SCHEDULER_ALGORITHMS.md` 참고.

## 입력 파일

**작업 트레이스 CSV** (헤더): `pod_name, pod_type, project, namespace, user_team, start_time, finish_time, accelerator_count, computing_level, gpu_utilization, accelerator_flavor, preemption`

**서버 CSV**: `server_name, accelerator_count, accelerator_type`
...
- **플랫폼 분기**: 시간 변환 등에서 `localtime_s`(Windows) vs `localtime_r`(Linux)처럼 `#ifdef _WIN32`로 분기한다. 새 시스템 호출 추가 시 양쪽 모두 처리할 것.

## 알려진 제한사항
1. Linux에서 선점(preemption) 기능 미작동 (스레드 유무에 따른 결과가 동일하다고 기록됨).
2. `scheduler_fare_share` 미구현.
3. 대용량 작업 파일은 메모리 사용량 주의.
