...
\caption{강화판 ablation --- 6구성 각각에서 고정 $\alpha$를 0.01--2.0로 스윕한 하위 1\% 순서 공정성(빨강) 대 무튜닝 SAFA(파란 점선). 부하가 존재하는 5구성 전부에서 \textbf{어떤 고정 $\alpha$도 무튜닝 선을 넘지 못하고} 좁은 띠에 갇힌다(1024 단일은 저부하라 전부 $p1{=}100$).}
...
\caption{합성 계수 $\pm50\%$ 교란에 대한 강건성(512 이종). 막대=baseline $p1$, 수염=교란 전 구간의 최소/최대. 베이스라인은 모두 $\approx0$, 고정 SAFA $\approx37$, 무튜닝 SAFA $\approx50$로 \textbf{주 대비가 교란에 거의 불변}이다.}
...
\begin{table}[htb!]
\centering
\caption{Helios(Venus, 125k 작업) 과부하 이종 일반화 --- 굶주림 비율 \texttt{lt50\%}(낮을수록 공정; 그림 \ref{fig:helios}).}
\label{tab:helios}
\footnotesize
...
\caption{Helios(Venus, 125k 작업) 과부하 이종에서의 굶주림 비율(\texttt{lt50\%}, 낮을수록 공정). 무튜닝 SAFA가 7.3\%로 정보-우위 그리디(LAS·Kueue·FGD, 20.8--21.9\%)의 약 1/3 --- 아사 예방이 독립 대규모 트레이스에서 재현된다.}
\label{fig:helios}
\end{figure}
...
\caption{배치 무관성 --- SAFA(무튜닝)를 4개 코어 배치(most-allocated/compact/round-robin/MCTS) 위에 얹었을 때의 하위 1\% 순서 공정성. 이종 과부하에서 \textbf{4개 배치 전부 $p1\ge50$}이며(SJF·LAS는 모든 배치에서 0), SAFA의 이점이 배치와 직교함을 보인다.}
\label{fig:placement}
\end{figure}
