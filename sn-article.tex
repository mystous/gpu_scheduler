...
GPU 클러스터의 작업 스케줄링은 시스템 최적화의 핵심 문제다. GPU를 얼마나 빠짐없이 채워 쓰느냐가 곧 운영 비용과 연구 비용을 좌우하기 때문이다. 게다가 GPU는 어느 조직에서나 늘 모자라는 자원이어서, 더 나은 스케줄링을 찾으려는 연구가 오랫동안 이어져 왔다. 규모가 큰 일부 조직은 수백~수천 장의 GPU를 고대역폭 인터커넥트로 묶어 전용 ``하이퍼클러스터''를 꾸린다. 그러나 이는 비용과 확장성 부담이 클 뿐 아니라, 클러스터 사이에 자원이 조각나는 단편화(fragmentation)를 부른다\cite{athlur2022varuna}. 프로덕션 트레이스를 분석한 연구도 같은 곳을 가리킨다. 여러 작업이 GPU를 부분(fractional)씩 나눠 쓰는 공유 환경에서는 단편화가 더 심해져, 수백 장의 GPU가 놀고 있는데도 새 작업에 배정되지 못한다\cite{weng2023beware}. 모델이 커지고 워크로드가 다양해질수록, 이런 자원을 효율적으로 굴리는 스케줄링은 더욱 중요해진다. 그러나 모든 상황에 통하는 해법은 없다. 완료 시간을 미리 알 수 없는 작업을 큐에 배정하는 문제 자체가 NP-hard 조합 최적화이고\cite{ullman1975np}, 실제 스케줄링은 flow shop·job shop·단일 기계·open shop·$k$-기계 병렬 스케줄링이 뒤섞인 형태여서\cite{b1} 대개 근사 알고리즘에 기댈 수밖에 없다\cite{b2}.

그럼에도 딥러닝 작업을 위한 스케줄러 연구는 꾸준히 발전해 왔다. Ye 등\cite{b3}은 이 분야를 두 축으로 정리한다. 하나는 워크로드 유형(학습·추론 등)에 따른 분류이고, 다른 하나는 효율성·공정성·마감 기한 같은 목표에 따른 분류다. 이들은 사용률·공정성·완료 시간에서 고루 앞선 Sia\cite{b4}와 다중 상태 프로파일링으로 성능을 끌어올린 Lucid\cite{b5}를 대표적인 최신(state-of-the-art) 스케줄러로 꼽으며, 오늘날 GPU 스케줄러 대부분이 이 계열의 변형이다. 다만 이들은 공통된 부담을 안고 있다. 워크로드 특성이 달라지면 정책이나 스케줄러를 다시 맞춰야 한다는 점이다.

이렇게 최근 제안된 GPU 스케줄러들도 저마다 한계를 안고 있다. 예컨대 learning-to-rank 기반 LLM 스케줄러에서는 순위 지표인 Kendall's Tau가 종단 간(end-to-end) 성능을 제대로 반영하지 못하는 경우가 보고됐다. 가장 짧은 70\% 요청의 내부 순서를 무작위로 섞어도 Tau는 0.5로 유지되지만, 정작 지연 시간은 크게 늘어난다(ShareGPT의 Llama-8B 기준 1.8배). 게다가 이 방법은 chunk-prefill이나 prefill-decode 분리(disaggregation) 같은 최신 LLM 서빙 최적화와 잘 맞물리는지가 아직 충분히 검증되지 않았다\cite{fu2024efficient}. 단편화를 직접 겨냥한 스케줄러에도 비슷한 사각지대가 있다. GPU 공유 워크로드용 Fragmentation Gradient Descent(FGD)\cite{weng2023beware}는 스케줄러 독립성을 지키려고 단편화를 ``한 스텝(one step)'' 범위로만 좁게 측정하는데, 이 좁은 시야는 자원이 넉넉하고 단편화가 낮은 초기 단계에서 오히려 차선(suboptimal)의 선택을 낳을 수 있다.

자원 할당을 정교하게 최적화하는 스케줄러일수록 복잡성과 한계도 함께 커진다. 예컨대 Optimus\cite{peng2018optimus} 같은 동적 스케줄러는 온라인 성능 모델로 학습 속도와 수렴을 추정한다. 효과적이긴 하나 예측 오차에 민감해서, 특히 수렴 추정에서 최대 20\%의 오차가 관찰됐다. 또 자원 구성을 바꿀 때마다 체크포인트를 거쳐야 해 오버헤드가 따르며, 워커나 파라미터 서버가 수백 개에 이르는 작업에서는 그 부담이 상당하다. 한편 Themis\cite{mahajan2020themis}는 공정성과 배치 민감성(placement sensitivity)을 노리지만, 그 대가로 구조와 계산이 크게 복잡해진다. 중앙 ARBITER가 앱이 제출한 입찰가(bid)를 받아 라운드마다 자원을 경매하는 구조인데, 부분 할당 경매에 솔버(예: Gurobi)를 쓰기 때문에 계산 비용이 크다(경합이 심하면 95퍼센타일 1,398ms). 더구나 그 효과는 앱이 제출하는 복잡한 입력 --- 비이상적 배치로 인한 속도 저하까지 정밀하게 모델링한 완료 시간 공정성(finish-time fairness) 추정값 --- 에 의존한다.

이 접근법들에는 한 가지 공통점이 있다. 모두 큐 \textit{바깥}의 정보 --- 순위 학습 모델, 성능·throughput 프로파일, 실행시간 추정, 경매 솔버 입력 --- 에 의존하며, 이런 부차 정보에는 저마다 프로파일링 비용·예측 오차·계산 오버헤드·전용 인프라가 따라붙는다(\S\ref{Related}). SAFA는 정반대로 간다. \textit{큐에서 직접 관측되는 정보}(대기 시간·자원 요구량·상대 나이)만으로 동작해, 기존 스케줄러를 대체하는 대신 보완한다. SAFA는 NP-hard 문제를 직접 풀지 않는다. 대신 어떤 스케줄러든 성능을 떨어뜨리는 핵심 병목 --- 자원 단편화에서 비롯되는 HOL 블로킹(Head-of-Line blocking) --- 만을 큐 전처리로 걷어낸다. 코어 스케줄러는 손대지 않은 채 HOL 블로킹이 일으키는 큐 지연과 도착 순서 불공정을 완화하는 것이다(처리량 자체를 높이는 것이 아니라, 같은 처리량 아래에서 대기와 공정성을 다시 분배한다).

이러한 단편화에서 비롯되는 HOL 블로킹\cite{karol1987hol}(선두 작업이 막혀 뒤따르는 작업까지 지연되는, 패킷 스위칭에서 비롯된 고전 현상)은 GPU 서버 풀 운영에서 구조적으로 발생한다(\S\ref{algo:intro}). 백필을 허용하는 스케줄러에서도 큰 작업이 선두에서 막히면 후속 작업의 배치가 밀린다. 결국 배치 정책과는 별개로, 어드미션 순서 단계에서 개입할 필요가 있다는 뜻이다.
...
본 절은 관련 스케줄러를 \textbf{의사결정에 동원하는 정보}를 기준으로 분류한다. 가르는 축은 단순하다. SAFA처럼 \textit{큐에서 직접 관측되는 정보}(대기 시간·자원 요구량·큐 위치·관측된 누적 서비스)만으로 동작하는가, 아니면 그 \textit{바깥의 부차 정보}에 의존하는가다. 부차 정보는 다시 셋으로 나뉜다. (i) 실행시간 추정 또는 순위 예측(SJF·EASY 백필링·learning-to-rank, \S\ref{SCRANK}·\S\ref{EASY}), (ii) 잡별 성능·throughput 프로파일(Optimus·Lucid·Sia, \S\ref{Optimus}·\S\ref{LU}·\S\ref{SI}), (iii) 라운드 솔버·경매 입찰(Themis·Sia, \S\ref{Themis}·\S\ref{SI})이다.

반대편의 \textit{큐 관측 정보만} 쓰는 무정보 진영에는 SAFA 외에도 두 기법이 있다. 누적 서비스 기반 LAS(Tiresias\cite{gu2019tiresias})와 게이트 계층 쿼타(Kueue, \S\ref{KUEUE})다. LAS는 관측된 실행량만 쓰므로 무정보이지만, 비선점 환경에서는 대기 작업의 누적이 0이라 비차단 FCFS로 퇴화한다(\S\ref{eval:impl}). 이 둘은 SAFA와 같은 층위의 비교군, 곧 어드미션 순서의 대안이다. 단편화 인지 배치(FGD, \S\ref{FGC})도 같은 무정보 진영이지만, \textit{배치 단계}에 속하는 직교 베이스라인이라는 점이 다르다. SAFA는 이들과 같은 ``큐 관측 정보만'' 진영에 있으면서, 배치(FGD)나 쿼타(Kueue)가 아니라 \textit{기아 방지 큐 재정렬}이라는 직교 축을 공략한다.

아래에서는 SAFA와 같은 큐 관측 정보 진영(Kueue·FGD)부터 시작해, 부차 정보 의존이 커지는 순서(실행시간 추정 → 프로파일 → 솔버·탄력)로 각 기법을 살펴본다. 이 분류는 \S\ref{baseline-rationale}에서 실험 포함·제외 기준으로 정식화되어 표 \ref{tab:related}에 요약되며, 결정 변수가 우리 고정요청 트레이스로 정의되지 않는 탄력·프로파일 의존 SOTA(Sia·Optimus·Pollux 등)를 동일 엔진 비교에서 제외하는 사유도 함께 밝힌다.

\subsection{Kueue}\label{KUEUE}
Kueue\cite{kueue2024}는 쿠버네티스 네이티브 큐잉 시스템으로, 작업을 suspend·재개하며 쿼타(quota) 기반 공정 공유를 제공한다. 코어 스케줄러를 교체하지 않고 게이트 계층에서 동작해 SAFA와 같은 레이어(큐 관측 정보만)에 속하며, 본 논문의 가장 직접적인 프로덕션 표준 베이스라인이다. 다만 공정성이 쿼타 분배에 한정되어 HOL 블로킹 기아를 유계로 보장하거나 단편화를 해소하지 않으며, 쿼타 공정성은 다중 VC 경합에서 발현되므로 트레이스의 VC가 하나뿐이면 FIFO로 퇴화한다.

\subsection{FGD}\label{FGC}
GPU 공유 클러스터에서는 작업이 GPU를 부분(partial)씩 차지하면서 단편화가 심해져(트레이스에서 21--42\%), 수백 장의 GPU가 할당 불가 상태로 남는다. FGD\cite{weng2023beware}는 이를 재는 통계적 지표를 도입한다 --- 무작위로 뽑은 작업이 들어가지 못하게 될 GPU의 기대량으로 단편화를 정량화하는 것이다. 그리고 이 값의 증가가 가장 작은 노드에 작업을 배치해 단편화가 새로 생기는 것을 막는다(프로덕션 트레이스에서 미할당 GPU 최대 49\% 감소). FGD는 잡별 프로파일 없이 큐·노드의 현재 점유만으로 \textit{배치}를 결정하므로 SAFA와 같은 큐 관측 정보 진영이되, 같은 문제(단편화)를 \textit{직교 축}에서 공격한다 --- FGD는 \textit{배치 시점}에 단편화를 회피하고, SAFA는 \textit{큐 순서}에서 단편화 피해자(기아 작업)에게 우선권을 준다(자원 적합도 $R$, \S\ref{algo:intro}). 본 논문은 FGD를 배치 베이스라인으로 포함하며(\S\ref{eval:sim}), 그 중앙값 단축이 배치가 아니라 비차단 큐 규율에서 나오고 배치 축은 본 과포화 레짐에서 무변별함을 보여(\S\ref{eval:main}) 효율(배치)과 공정성(큐)이 직교 축임을 실증한다.

\subsection{Efficient LLM Scheduling by Learning to Rank}\label{SCRANK}
\textit{부차 정보 (i): 실행시간 추정/순위 예측.} LLM 서빙은 출력 길이를 알 수 없어 FCFS를 쓰며 심각한 HOL 블로킹을 겪는다. Efficient LLM Scheduling by Learning to Rank\cite{fu2024efficient}는 정확한 길이 예측 없이 출력 길이의 \textit{상대 순위}만으로 SJF/SRTF를 근사할 수 있음을 보이고, 소형 보조 모델의 learning-to-rank로 순위를 예측하며 긴 요청을 위한 기아 방지를 더한다(단, 높은 Kendall's Tau가 항상 낮은 지연을 보장하지는 않는 한계를 저자도 지적). 이는 ``길이 추정 기반 SJF + 기아 방지''라는 구조에서 SAFA와 목적을 공유하나, 출력 길이라는 미래 정보의 순위 예측에 의존하고 LLM 요청(iteration) 단위에서 동작한다 --- SAFA는 클러스터 갱(gang) 스케줄링에서 작업 duration을 전혀 쓰지 않고 큐 나이 승급만으로 기아를 막는다.

\subsection{EASY Backfilling}\label{EASY}
EASY 백필링\cite{mualem2001backfilling}은 HPC 배치 스케줄링에서 수십 년간 검증된 표준 기법이다. 큐 선두 작업을 위해 자원을 예약(reservation)하되, 그 예약을 건드리지 않는 후속 작업은 앞당겨 실행(backfill)한다. 이렇게 선두 작업의 기아를 막으면서 활용도도 함께 끌어올린다. 그러나 EASY는 각 작업의 실행 시간 추정에 의존하며, 사용자 추정은 흔히 2--10배의 오차를 갖는다. 흥미롭게도 과대추정이 오히려 성능을 개선하는 ``과대추정 역설''이 보고되었고(Zotkin--Keleher\cite{zotkin1999overestimation}의 선행 관찰, Mu'alem--Feitelson\cite{mualem2001backfilling}의 체계화), 그 원인은 통설이던 예약 보수화가 아니라 heel-and-toe 동역학임이 규명되었으며\cite{tsafrir2006dynamics}, 시스템 측 예측이 더 낫다는 반박으로 이어졌다\cite{tsafrir2007backfilling}. 본 논문은 EASY를 완벽-추정 변형으로 평가에 포함한다(과대추정 역설\cite{zotkin1999overestimation,mualem2001backfilling} 때문에 ``상한선''이라 부르지 않는다 --- \S\ref{eval:impl}). SAFA는 예약 대신 나이(age) 기반 승급으로 기아를 막아 duration 추정 자체가 불필요하다는 점이 다르다. 보장 수준도 다르다 --- EASY의 예약은 \textit{큐 선두 1건}의 시작 시각 $T$가 backfill로 지연되지 않음을 보장하지만(선두 외 작업까지 예약하는 것은 conservative backfilling\cite{feitelson1998conservative}의 몫), SAFA의 나이 승급은 \textit{큐 정체}만 절단할 뿐 개별 작업 수준의 시각 보장이 없다(\S\ref{SAFA}에서 정직히 한정).

\subsection{Optimus}\label{Optimus}
\textit{부차 정보 (ii): 잡별 성능 프로파일.} Optimus\cite{peng2018optimus}는 온라인 성능 모델로 모델 수렴(필요 스텝)과 자원별 학습 속도를 예측해 평균 JCT를 최소화하도록 자원을 동적 할당하는 DL 클러스터 스케줄러다(Kubernetes에서 DRF·Tetris 대비 JCT 139\% 개선). 그러나 정확한 온라인 성능 모델과 탄력적(elastic) 파라미터 서버 구조를 전제하므로 애플리케이션 수정과 예측 정확도에 의존한다. SAFA는 그런 모델·앱 협조 없이 대기 큐와 서버 상태만으로 동작해 정보 요구가 최소다.

\subsection{Lucid}\label{LU}
Lucid\cite{b5}는 선점·프레임워크 변경 없이 동작하는 비침습적 딥러닝 스케줄러로, 잡별 utilization 프로파일에 기반해 작은 작업을 겹쳐(collocation) 평균 JCT를 줄인다. 비침습 철학은 SAFA와 같으나 프로파일링을 요구한다. 우리의 대규모 시뮬레이션에서 Lucid는 작은 작업을 즉시 배치해 중앙값 대기를 극소화하지만 긴 작업의 꼬리 대기를 방치하는 경향을 보였는데, SAFA는 바로 이 최악 대기(기아) 억제에 초점을 둔다.

\subsection{Themis}\label{Themis}
\textit{부차 정보 (iii): 라운드 솔버·경매.} Themis\cite{mahajan2020themis}는 완료 시간 공정성(finish-time fairness) $r=T_{sh}/T_{id}$(공유 대 전용 1/N 클러스터 완료시간 비)를 장기 공정성 지표로 도입하고, 중앙 ARBITER가 라운드 단위 경매로 최대 $r$을 최소화한다(앱이 추정 $r$로 입찰; 전략 무용성·파레토 효율 보장). $r$은 속도 저하(slowdown) 기반 공정성으로 SAFA가 설계 출발점에서 참조한 PS-공정 계보에 속한다. 다만 Themis는 솔버 경매(경합 시 p95 1,398ms)와 앱이 제출하는 $r$ 추정에 의존하는 반면, SAFA는 측정된 큐 나이 통계만으로 정규화되는 $O(n)$ 경량 연산이다.

\subsection{Sia}\label{SI}
Sia\cite{b4}는 이종 GPU 클러스터의 자원 적응형 딥러닝 작업을 위해 GPU 타입·수량·배치 크기를 라운드마다 ILP로 재최적화하는 탄력(elastic) 스케줄러로, 각 작업의 throughput-scaling 프로파일을 요구한다. 이 프로파일 의존은 후속 연구가 비용·오차를 보고한 지점이다 --- Arena는 선형 throughput 외삽이 최대 2.12$\times$ 오차, SchedMate는 프로파일링을 우회하면 JCT 13--20\% 감소, RLTune은 블랙박스·신규 잡에 부적용\cite{arena2026,schedmate2025,rltune2025}. 본 연구는 Sia를 동일 엔진 비교에서 \textit{제외}한다: 그 결정 변수(goodput-최적 GPU 수·타입 구성)가 고정 요청 갱(gang) 트레이스에는 정의될 수 없고, 프로파일을 합성 주입한 포팅은 원 논문·후속 비평\cite{arena2026,rubick2025}과 수치가 어긋나기 때문이다(\S\ref{baseline-rationale}). 따라서 Sia는 Arena·Rubick 등과 함께 ``결정 변수를 트레이스로 정의할 수 없는 탄력·프로파일 의존 SOTA''로 관련 연구에 둔다. 고정 요청 갱의 \textit{작업 간} 큐 순서를 예측·프로파일 없이 단일 승급 $O(n)$으로 다루는 SAFA의 \textit{무정보 경량성}이 이들과 구별되는 위치다.
...
\textbf{(b) 결정 변수가 트레이스에 부재.} Sia\cite{b4}(goodput-최적 GPU 수·타입), Gavel\cite{narayanan2020gavel}(잡별 타입-처리량 행렬 --- 본 엔진의 타입 속도계수는 \textit{전 작업 공통}이라 잡별 행렬이 퇴화), Optimus\cite{peng2018optimus}(탄력 worker/PS), Pollux\cite{qiao2021pollux}(탄력 goodput), Arena\cite{arena2026}(적응 병렬화), Rubick\cite{rubick2025}(실행 계획 재구성)의 결정 변수는 \textit{잡별 성능 프로파일} 또는 \textit{작업 내부 병렬화 계획}이라, 고정 요청 갱 트레이스에는 존재하지 않아 정의할 수 없다(완벽한 프로파일을 줘도 마찬가지). 제외 사유는 ``재구성한다''가 아니라(Lucid도 일부 수행) \textbf{결정 변수가 우리 트레이스로 정의되지 않는다}는 데 있다. Sia를 탄력 재구성을 끈 rigid 모드로 강등하면 그 유일한 기여(goodput-최적 재구성)가 사라져 우리가 이미 포함한 FIFO·SJF와 동치가 되므로 별도 행 게재는 오해를 부르며, 합성 프로파일 포팅의 수치가 원 논문·후속 비평과 어긋남은 \S\ref{SI}와 보충자료 \texttt{FINDINGS.md}에서 밝힌다.
...
딥러닝의 급격한 발전은 컨테이너\cite{b7} 기반의 재현 가능한 공유 학습 환경에 크게 빚지고 있다. Docker Hub\cite{b8}나 Hugging Face\cite{b9} 같은 서비스가 ML 프레임워크·라이브러리·신경망 아키텍처를 손쉽게 공유하게 하면서, 연구자들은 재현성 확보에 드는 시간을 덜고 일관된 환경에서 알고리즘 자체에 집중할 수 있게 되었다. 그러나 이 편의에는 대가가 따른다. 학습 환경이 추상화되는 과정에서 모델의 GPU 의존적 구성 요소도 함께 가려지며, 바로 이 추상화가 역설적으로 GPU 서버 풀의 비효율을 낳는다.

GPU 접근이 카드 단위로 추상화되어 있어, 컨테이너는 보통 GPU를 통째로 점유한다. 그 결과 하드웨어가 할당률을 높이는 타임 슬라이싱을 지원하더라도 학습이나 서버 풀 운영 과정에서 이를 유연하게 활용하기 어렵다. 여기에 모델 규모가 커지면서 한 작업이 여러 GPU를 동시에 요구하는 일이 보편화되었다. 운영 중인 서버 풀에는 완료 시간도 GPU 요구량도 제각각인 작업이 끊임없이 제출되고, 카드 단위의 불가분 점유와 맞물려 풀 내부에 단편화가 쌓인다. 단편화는 다음과 같은 상황에서 발생한다.
...
그림 \ref{fig:concept}(a)와 (b)는 각각 GPU 서버 풀과 작업 대기 큐의 상태를 보여준다. most-allocated 방식을 사용하는 스케줄러는 작업 $\textit{t}_{0}$과 $\textit{t}_{1}$을 할당할 수 있다. 그러나 $\textit{t}_{2}$는 다른 작업들이 완료되어 GPU 슬롯이 비워지기를 기다려야 하므로 스케줄링할 수 없다. 이는 작업 $\textit{t}_{3}$, $\textit{t}_{4}$, $\textit{t}_{5}$가 그 이후에야 스케줄링될 수 있음을 의미한다(그림 \ref{fig:concept}(c)).

그림 \ref{fig:concept}(c)에 나타난 것처럼, $\textit{t}_{3}$, $\textit{t}_{4}$, $\textit{t}_{5}$를 스케줄링하기에 충분한 GPU 슬롯이 있음에도 이 작업들은 $\textit{t}_{2}$ 때문에 스케줄링되지 못한다. 이때 대기 큐에는 그림 \ref{fig:concept}(d)와 같이 $\textit{t}_{2}$--$\textit{t}_{5}$가 잔류하며, 선두의 $\textit{t}_{2}$가 자리를 얻기 전까지 뒤의 세 작업이 함께 묶인다. 이것이 HOL 블로킹이다. 이는 앞서 할당된 작업들이 GPU 서버 풀을 단편화시키기 때문이다. GPU 서버 풀의 단편화는 운영 중 지속적으로 발생하며 간헐적으로만 해소된다. 최악의 경우는 모든 GPU 서버에 7장의 GPU가 남아 있는데 HOL 블로킹 작업이 8장을 요청하는 상황이다.

이 경우 GPU 서버 풀은 어떤 작업이 완료되기 전까지 낮은 GPU 할당률에 머무른다(그림 \ref{fig:concept}(c)). 다만 이 할당률 손실은 \textit{중간 부하}에서 두드러지는 증상이다. 작업이 끊임없이 제출되어 클러스터가 포화되는 \textit{과부하}에서는 빈 슬롯이 곧 다른 작업으로 채워져 할당률은 높게 유지되며(\S\ref{eval:main}에서 모든 정책이 약 99\%), 이때 HOL 블로킹의 비용은 할당률 손실이 아니라 막힌 작업들의 \textbf{큐 지연}과 \textbf{도착 순서 불공정}으로 전환된다. 어느 부하에서든 근본 병목은 단편화에서 비롯된 HOL 블로킹이며, 본 연구는 이를 큐 전처리로 완화한다.

이 HOL 블로킹은 정책 선택만으로 제거되지 않는다. 여러 스케줄링 정책을 동일 조건에서 비교하면, 중앙값 대기와 순서 공정성·최악 꼬리(기아)는 대체로 서로 맞바꿔지는(trade-off) 경향을 보인다. 다만 이는 법칙이 아니라 \textit{측정으로 확인되는 경험적 경향}이다 --- 실제로 본 평가에서도 일부 셀은 두 축이 함께 개선되어, 이종 512의 SAFA는 FIFO 대비 중앙값과 최악 꼬리를 모두 줄인다. 따라서 본질적 과제는 ``어느 균형점을 택할 것인가''에 더해 ``그 균형점을 워크로드가 바뀔 때마다 재튜닝 없이 유지할 수 있는가''로 귀결된다. 이 물음이 곧 SAFA의 설계 동기다. 본 논문은 이 트레이드오프 구조를 \S\ref{eval:main}의 대규모 시뮬레이션에서 정량적으로 확인한다.


왜 이 단편화가 고전적 CPU 스케줄링의 기아와 \textit{질적으로} 다른지 짚어 둘 필요가 있다. 단일 자원(CPU 시간)을 다루는 고전 스케줄링에서 기아는 \textit{낮은 우선순위의 장기 미배치} 문제이며(동기화 현상인 priority inversion과는 별개 개념이다), 시분할(time-slicing)과 선점으로 굶은 작업에 언제든 한 조각의 CPU를 떼어 줄 수 있어 priority aging만으로 원리적으로 해소된다 --- 한 작업이 받는 자원은 연속적으로 분할 가능하다. 그러나 GPU 갱(gang) 스케줄링의 HOL 블로킹은 \textit{공간적·불가분(all-or-nothing) 자원 단편화}에서 비롯된다. 8-GPU 작업은 8장이 \textit{동시에·한 갱으로} 확보되어야만 실행되며(부분 실행 불가), 빈 GPU가 여러 서버에 7장씩 흩어져 있으면 총량이 충분해도 배치할 수 없다. 게다가 GPU 작업은 비선점이 사실상의 표준이다 --- 학습 작업의 체크포인트·재적재 비용이 크고(리젝 ②의 GPU 선점 비현실성과 일치), 본 환경도 전 정책 비선점이다. 따라서 굶은 큰 작업에 ``자원의 한 조각''을 떼어 주는 CPU식 완화가 불가능하며, 기아 해소는 오직 \textit{큐 순서}를 통해 ``그 작업이 들어갈 연속 슬롯이 비는 순간까지 후속 작업이 자리를 영구히 잠식하지 못하게 하는'' 방식으로만 가능하다. 정확히 하면, 불가분·비선점·공간 단편화의 결합 자체는 새롭지 않다 --- 공간 공유 병렬 작업 스케줄링(JSSPP 계보\cite{feitelson1996toward})의 창립 가정이고, Maui의 XFactor·대기시간 가중\cite{jackson2001maui}과 Slurm age factor가 바로 그 맥락에서 aging을 수십 년 운용해 왔다. 자원 적합도를 \textit{작업 순서 결정}에 결합한 가장 가까운 선행은 Tetris\cite{grandl2014multi}로, 다차원 자원 정렬(alignment) 점수와 잔여 작업 패킹을 결합하고 효율--공정 트레이드오프를 \textit{명시적 노브}로 조정한다. 본 연구가 이 고전 맥락에 더하는 새 요소는 두 가지로 한정된다: \textit{노드 경계 패킹의 자리 적합($R$)을 aging에 결합}하되 Tetris처럼 배치 점수가 아니라 \textit{큐 순서에만} 적용하는 것(배치 정책은 불변)과, 그 계수를 \textit{큐 통계로 정규화해 무튜닝으로} 만드는 것(Tetris·Maui의 명시적 가중 노브와 대비)이다(각각 \S\ref{eval:ablation:comp}·\S\ref{eval:ablation:grid}에서 성분 격리). SAFA가 선점이나 자원 분할 없이 큐 순서만으로 동작하도록 설계된 이유가 여기에 있다.
...
우리는 HOL 블로킹 해소 메커니즘을 분석하고 빠르게 반복 개발하기 위해 작업 스케줄링용 자체(in-house) 시뮬레이터를 개발했다. 이 시뮬레이터는 작업 로그를 파싱해 여러 스케줄링 정책으로 재생하며, 기존 메커니즘을 건드리지 않고 새 스케줄링 방법론을 끼워 넣을 수 있다. 이러한 모듈성은 큐 조정이 배치 스케줄링 방식과 독립적으로 동작함을 보여준다.

\section{Resolving Head-of-Line Blocking in the Job Queue}\label{algo:intro}
우리는 작업 대기 큐의 HOL 블로킹을 해소하기 위한 기아 인지 공정 나이가중(Starvation-Aware Fair Aging) 알고리즘 SAFA를 제안한다. 용어를 정리해 두면, 본 논문의 ``gang''은 GPU 스케줄링 문헌의 관용대로 \textit{전부-아니면-전무로 동시 할당되는 고정 GPU 요청}을 가리키며, Feitelson--Rudolph 고전 분류\cite{feitelson1996toward}의 rigid job에 해당한다(조율된 시분할을 뜻하는 Ousterhout류 고전 gang scheduling\cite{ousterhout1982gang}과 구별). SAFA는 나이 가중 계수 $\alpha$로 트레이드오프를 제어하는데, 기본형은 $\alpha$를 고정하지만(SAFA(고정 $\alpha$)), 본 논문의 SAFA는 계수 튜닝을 제거해 $\alpha$를 관측된 큐 나이 통계로 정규화한다(무튜닝). SAFA는 배치 단계 앞에서 어드미션 순서를 결정하는 계층으로서, 스케줄러 실행 전 작업 대기 큐를 재정렬해 단편화에서 비롯되는 HOL 블로킹을 직접 완화한다.
\subsection{Starvation-Aware Queue Adjustment}
큐 조정의 목표는 처리량을 높이는 것이 아니라 동일 처리량 아래에서 HOL 블로킹으로 막힌 작업들의 큐 지연을 줄이고 도착 순서 공정성을 지키는 것이다. 큐를 조정하면 막힌 작업들이 스케줄링될 수 있으나, 최악의 경우 막힘을 유발하는 가장 큰 작업 자체는 계속 자리가 비기를 기다린다 --- 따라서 본 알고리즘이 방지하는 것은 \textit{큐가 한 작업에 무한정 막혀 후속이 영구히 적체되는 상태}이며 개별 대형 작업의 완료를 보장하지는 않는다(이 정직한 범위는 \S\ref{SAFA}·\S\ref{eval:main}에서 명확히 한다). 우선순위를 임의로 조정하면 새 공정성 문제가 생길 수 있으므로 나이 가중으로 그 트레이드오프를 제어한다.
...
SAFA는 코어 스케줄러와 독립적으로, 주기적으로 동작하는 스케줄러보다 한 발 앞서 실행된다. 동작 결정에 작업 대기 큐와 서버 풀의 현재 할당 상태만 사용하므로, 코어 스케줄러의 종류나 방식과 무관하게 큐를 다듬는 전처리기로 기능한다. 다만 SAFA가 대기 큐를 직접 관리하는 만큼, 큐 스케줄링을 자체적으로 수행하거나 누적 지표·문맥에 의존하는 스케줄러는 적용 대상에서 제외한다.
...
$\alpha$는 HOL 블로킹 제거에 도움을 주지만, 값이 지나치게 크면 막혀 있던 작업이 다른 작업을 과도하게 추월하여 새로운 불공정을 낳을 수 있다. 적절한 $\alpha$를 정하려면 이 트레이드오프를 정량화해야 한다. 이를 위해 각 작업의 누적 대기 시간을 요청 가속기 수별로 묶고, 그 변화량을 누적 평균 거리(accumulated average distance, AAD)로 정의한다.
\begin{equation}
...
두 지표는 작업 대기 시간의 평균 편차와 최대 편차를 함께 포착한다. AAD가 전반적 대기 쏠림을, AMD가 특정 클래스의 최악 대기를 잡아내므로, 둘을 함께 최소화하면 평균적 공정성과 기아 억제를 동시에 겨냥할 수 있다.
...
이 무튜닝 적응의 대가는 작은 중앙값 양보이며, 그 대신 부하·이질성 전반에서 일관되게 높은 공정성을 얻는다는 점을 \S\ref{eval:main}에서 정량적으로 보인다.
...
계수 $\alpha$를 제거한 뒤 SAFA에 남는 소수의 내부 상수가 또 다른 숨은 노브가 아님을 보이기 위해, 별도의 1차원 민감도 분석을 수행했다. 그림 \ref{fig:sensitivity}는 C++ 시뮬레이터로 12개 구성을 스윕한 결과를 보여준다 --- 자원 적합도 페널티 $R$(0.05--0.3)과 우선순위 밑 $P$(1.5--4)는 makespan 변동이 $\pm$2\% 이내로 둔감했고, 큐 재정렬 창 $m$만 $m$=1에서 +8.9\%로 민감했으며 $m\ge 2$에서는 1.4\% 이내였다(본 연구는 안전값 $m$=3 채택). 즉 이 상수들은 \textit{처리량(makespan)} 측면에서 넓은 범위에 둔감하다. 다만 이 둔감성은 처리량에 한정된 진술이다 --- 주축 지표인 \textit{순서 공정성}(p1)으로 보면 $R$ 감쇠는 둔감하지 않고 효율--공정 균형점을 옮기는 설계 상수이며(\S\ref{SAFA}·\S\ref{eval:robust:sens}에서 p1 기준으로 정량화), 본 절의 민감도는 ``숨은 노브가 makespan을 흔들지 않는다''는 처리량 안정성에 대한 것이지 모든 지표에 둔감하다는 뜻이 아니다.
...
평가는 다섯 단계로 전개된다. (i) \textbf{실험 설정}(\S\ref{eval:sim}): 시뮬레이터·지표·구성과 엔진 충실도 검증. (ii) \textbf{베이스라인 구현}(\S\ref{eval:impl}): 10개 동일 엔진 정책의 충실 구현. (iii) \textbf{주 결과}(\S\ref{eval:main}): Philly 111k를 6개 구성(256--1024 GPU $\times$ 단일·이종)에서 비교한 효율--공정성 지형. (iv) \textbf{Ablation}(\S\ref{eval:ablation}): 무튜닝 적응의 순효과를 고정 $\alpha$ 대비 격리. (v) \textbf{강건성과 일반화}(\S\ref{eval:robust}): 합성 계수 민감도($\pm50\%$), 독립 트레이스 재현, 통계적 유의성으로 주 결과의 견고함을 시험. (vi) \textbf{배포와 실측}(\S\ref{SAFA:k8s}·\S\ref{eval:b200}): 쿠버네티스 통합 경로와 B200 단일 노드 실 클러스터 실측으로 시뮬 경향을 교차 검증. 마지막으로 \S\ref{eval:threats}에서 남은 위협을 정직히 밝힌다.


\subsection{Scale generalization via discrete-event simulation}\label{eval:sim}
\S\ref{SAFA}의 $\alpha$ 탐색에 쓴 이산 시간 C++ 시뮬레이터는 8개 분포별 합성 트레이스에 한정되므로, 우리는 Philly 트레이스\cite{jeon2019philly} 전체(약 111k 작업; 본 스윕 트레이스의 최대 요청은 8 GPU --- 노드 경계 단위)를 256, 512, 1024 GPU 클러스터(단일·이종 구성)에서 평가하는 이산 사건 시뮬레이터를 개발했다. 시뮬레이터에는 \S\ref{eval:overhead}의 작업당 고정 오버헤드를 주입했다. 동일 엔진 비교에는 기준선 FIFO와 어드미션 순서 정책 SJF·LAS·Kueue·EASY·Themis·Lucid, SAFA 두 변형, 그리고 배치 축 참고로 단편화 인지 FGD까지 충실히 구현해 포함했다(저자 공개 코드가 있는 Tiresias·Kueue·FGD·Lucid는 그 알고리즘과 대조검증, \S\ref{eval:impl}). 잡별 성능 프로파일을 트레이스로 정의할 수 없는 탄력 스케줄러(Sia 등)는 동일 엔진에서 제외하고 관련 연구로 다룬다(\S\ref{baseline-rationale}).

구현 세부 세 가지를 명시한다. (i) 주 평가 엔진에서 작업 나이 $A$는 \textit{신규 작업이 도착할 때마다} 1씩 증가하는 도착 카운터로 측정된다 --- 이는 단위 차이를 넘어 \textbf{의미론 선택}이다: 같은 512 이종에서 나이를 wall-clock 초로 바꿔 재실행하면 p1이 50.5에서 39.3으로 내려가고 중앙값은 1.76M에서 1.22M초로 빨라진다(wall 나이는 장대기 작업의 절대 격차가 $A_{\max}$를 키워 승급 문턱을 높인다; 영향은 트레이스 의존적이어서 Alibaba에서는 98.4 대 97.7로 거의 같다). 본 논문은 \textit{큐 사건(도착) 기준} 나이를 설계 선택으로 채택하며 --- 기아 압력을 시간이 아니라 ``그 사이 몇 개가 들어왔는가''로 재는 것이 백로그 동역학에 정합한다 --- 배포 컨트롤러에서도 같은 의미론을 Pod 생성 순번으로 구현할 것을 권장한다(순번은 creationTimestamp의 \textit{순위}에서 유도되므로 컨트롤러 재시작 후에도 도착 \textit{순서}는 결정적으로 재구성된다 --- wall 값이 아니라 순서만 쓴다; 단 이미 어드미션되어 떠난 중간 도착은 재열거에서 빠지므로 상대 나이의 \textit{간격}까지 보존되지는 않고, 따라서 재시작 직후 승급 \textit{문턱}은 달라질 수 있으며 결정적으로 보존되는 것은 도착 순위다)(\S\ref{SAFA:k8s}의 B200 실측은 wall 나이 구현이나, 균일 도착 구간이라 두 의미론이 수렴한다) --- \S\ref{algo:intro}의 `10분당 1'(C++ 시뮬 설정)과는 단위만 다르며, 고정 $\alpha$는 \S\ref{eval:ablation}의 $\alpha$ 전 구간 스윕이 단위 차이를 흡수하고, 무튜닝 SAFA는 $A_{\text{ref}}$ 정규화로 나이 단위에 불변이다 --- 단 $A_{\text{ref}}=\max(1,\cdot)$의 바닥 때문에 이 불변성은 큐 평균 상대 나이가 1 단위 이상일 때 성립하며(도착 카운터·wall-clock 초 모두 과부하 큐에서 자명하게 충족), 평균이 1 미만으로 떨어지는 극저부하에서는 보너스 자체가 0에 가까워 무의미하다. (ii) 노드 용량(8)을 초과하는 멀티노드 gang 요청의 $R$은 8-슬롯 항목으로 클램프된다 --- 가장 나쁜 적합으로 일괄 취급되며, Helios·Alibaba의 대형 gang이 이 경로를 쓴다(노드-묶음 $R$ 일반화는 \S\ref{eval:threats}(3)의 향후 과제). (iii) base 우선순위 $P$는 큐 위치 60 이후 0으로 절단한다($1/2^{60}\!\approx\!8.7\times10^{-19}$로 수치상 0) --- 깊은 위치의 작업은 나이 보너스로만 승급 가능하며, 이는 C++ 시뮬의 재정렬 창($m\times$서버 수)과 같은 효과다.

시뮬레이터는 결정적 전체 트레이스 재생이므로 동일 입력에서 결과가 비트 단위로 재현된다(예외: Lucid는 난수 util 샘플링과 전역 슬로다운 상태에 의존해 하니스 구성에 따라 절대값이 미세하게 갈릴 수 있으며 --- 표 \ref{tab:sens}의 Lucid가 주 표와 다른 이유 --- 주 결과의 Lucid 값은 주 sweep 하니스를 기준으로 한다). 본 절의 모든 표·그림은 공개 스크립트(스윕·ablation·민감도·부트스트랩·충실도)와 그 실행·병합 절차를 문서화한 재현 가이드로 산출된다.\footnote{재현 절차와 스크립트 목록은 보충자료 \texttt{docs/REPRODUCE.md} 참조.}
...
이하 표의 큐잉 지연 절대값(과부하 구성에서 중앙값이 수일\,$\sim$\,수개월에 이르고 최악 대기는 그보다 길다)은 그 자체로 운영 현실을 나타내는 것이 아니라 \textit{스트레스 테스트의 산물}이다. 부하배수는 트레이스의 총 GPU 수요를 클러스터 용량으로 나눈 비로서 256 GPU에서 3.6$\times$, 512에서 1.8$\times$, 1024에서 0.9$\times$이며, 일부러 작게 잡은 클러스터에 트레이스를 실시간으로 재생해 지속적인 백로그를 강제로 만든 조건이다. 따라서 절대 초 수치는 이 과포화 설정에서 비롯되므로 의미 있는 것은 \textit{동일 조건에서 정책 간의 상대 비교}이지 절대값이 아니다. 실제 프로덕션 클러스터는 어드미션 컨트롤(admission control)로 큐 깊이를 제한하므로 수일·수개월의 절대 대기가 그대로 관측되지는 않는다 --- 본 설정은 그 안전장치를 일부러 제거해 HOL 블로킹·기아를 \textit{증폭}시킨 제어된 스트레스 프로브이며, 운영 예측이 아니라 정책의 \textit{꼬리 거동}을 분리해 드러내기 위한 것이다. 정상 부하(1024 GPU, 0.9$\times$)에서의 거동은 \S\ref{eval:main}에서 별도로 보고한다.
...
\item \textbf{EASY 백필링~\cite{mualem2001backfilling}}: FIFO$+$선두 작업 예약(실행 중 작업의 가장 이른 종료로 자리 확보 시점 $T$ 계산)$+$잔여 큐를 도착순 스캔해 $T$ 이전 종료 작업만 backfill. duration 추정을 쓰며 완벽 추정을 기본으로 둔다(과대추정이 이로울 수 있어 ``상한''이라 부르지 않는다). 본 표는 Mu'alem--Feitelson 허용 조건 (a)$T$ 이전 종료만 쓴 보수판이다; (a)$+$(b)$T$ 시점 잔여 슬롯을 모두 넣은 완전판은 단일에서 p1이 크게 오르고(256/512 단일 0$\to$54.2/74.1) 이종에서는 p1 0 유지, Alibaba에서 효율·공정이 함께 개선된다(\texttt{lt50} 15.3$\to$9.4; 보충자료 \texttt{easyfull\_all.csv}). 따라서 EASY 잠재력은 본 표보다 높게(특히 단일) 읽어야 하나 SAFA 대비 구도(이종·과부하 EASY p1 0 대 SAFA 50+)는 유지된다. 본 설정에서 EASY 중앙값 이득이 작은 것(단일 과부하 FIFO 대비 10--15\%)은 지속 과포화에서 예약이 backfill 여지를 선두 slack으로 제한하기 때문이다.
...
\item \textbf{Kueue~\cite{kueue2024}}: 테넌트(VC)별 \texttt{nominalQuota}를 정하고 under-served VC를 우선하되 유휴 용량은 다른 VC가 빌려 쓴다(cohort borrowing, 비차단 backfill). 프로덕션 admission 로직을 시뮬에 이식했다(Philly의 테넌트 식별자 15개, 최대 VC가 47\%; 수요 비례 쿼타라 본 행은 보수적 하한 운용). 이 borrowing·비차단 변형은 \textit{순서 공정성에 비관적}이라 --- StrictFIFO에 가깝게 운용하면 순서 공정성이 크게 개선되고(\S\ref{eval:b200} B200 실측의 Kueue \texttt{lt50\%} 4.4 상위권), 단일 VC StrictFIFO는 정의상 head-blocking FIFO와 동치라 본 표의 FIFO 행이 대리한다 --- 본 절 표의 Kueue p1은 ``Kueue의 한계''가 아니라 borrowing을 켠 처리량-우선 구성의 비용이다. 멀티-VC에서는 StrictFIFO라도 VC 간 인터리빙이 전역 도착순을 깨므로 전역 p1은 회복되지 않는다(per-VC 공정성과 전역 도착순 공정성은 다른 목표).
...
앞 비교의 고정 $\alpha$는 모든 구성 공통 단일값이었다. 더 강한 질문은 ``\textit{구성마다} $\alpha$를 최적으로 골라도 무튜닝이 따라잡히는가''이다. 6구성 각각 16점 그리드($\alpha\in[0.01,2.0]$ 부등간격\footnote{$\alpha$ 값: 0.01, 0.02, 0.05, 0.1, 0.139, 0.2, 0.3, 0.5, 0.7, 0.9, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0.})로 최선 고정 $\alpha$를 구해 대비했다(표 \ref{tab:ablation-grid}). 그리드 내 5개 부하 구성 전부에서 어떤 고정 $\alpha$도 무튜닝에 못 미친다(최선 대비 $+4.4$--$12.8$점, 예: 512 이종 39.1 대 50.5). 다만 그리드 하한이 결론을 만들지 않는지 검증했다 --- $\alpha_{\text{eff}}{=}1/(A_{\text{ref}}R_{\min})$가 과포화에서 $10^{-3}$ 이하로 내려가므로 $10^{-4}$까지 확장하면 고정 $\alpha$도 FIFO 쪽으로 올라간다(512 단일 $10^{-4}$ p1 87.5, $3{\times}10^{-4}$ 61.0). 즉 고정 $\alpha$ 역시 --- $w$와 마찬가지로(\S\ref{eval:ablation:comp}) --- FIFO와 그리디를 잇는 보간 노브이고 도달 불가능한 동작점은 없다. 무튜닝의 가치는 frontier 독점이 아니라 두 가지다. 첫째, \textbf{노브의 위험한 민감성 제거}: 고정 $\alpha$의 p1은 $10^{-4}$--$3{\times}10^{-3}$에서 87$\to$46으로 급변하는 절벽이고 그 위치가 워크로드 나이 스케일에 결합돼 트레이스가 바뀌면 동작점이 달라진다(\S\ref{sec:coeff}의 7배 변동과 같은 뿌리). 무튜닝은 $A_{\text{ref}}$ 정규화로 이 결합을 끊고 노브 없이 균형점(512 단일 53.1, 이종 50.5)에 도달한다. 미세 고정값 $3{\times}10^{-4}$도 세 구성(Philly 이종·Helios·Alibaba)에서 p1 48.8/66.9/62.5로 18점 출렁이는 반면, 무튜닝은 50.5/83.9/98.4로 노브 없이 더 공정한 균형점을 잡아 그리드 최선 고정값 대비로도 세 트레이스 전부에서 13--25점 상회한다. 둘째, \textbf{재튜닝 불요}: 같은 코드로 6구성·세 트레이스 전부에서 균형점이 유지된다. 그 대가는 작은 중앙값 양보(그리드 최선 대비 $q_{p50}$ $+18$--$32$\%)다.
...
동일 엔진 베이스라인 중 SJF·EASY·Themis는 완벽한 duration 예측(oracle)을, Lucid는 정확한 잡별 utilization 프로파일을 \textit{비용·오차 없이} 제공받는다 --- 이는 이들 정보-우위 정책에 \textit{유리한} 방향의 이상화이므로, 무정보 SAFA가 이들과 경쟁한다는 결론은 \textit{보수적}이다(편향이 SAFA에 불리). 이 결론이 합성 계수의 특정 선택에 의존하지 않음을 보이기 위해, 합성 파라미터를 각각 $\pm50\%$ 흔들어 512 단일·이종에서 \textbf{전 정책}을 재평가했다(표 \ref{tab:sens}, 그림 \ref{fig:sens-robust}). 타입 속도계수와 수명주기 오버헤드는 \textit{모든} 정책의 거동에 영향을 주므로 표의 10행(베이스라인 8개와 SAFA 두 변형) 전부를 교란 하에서 재실행했고, Lucid의 collocation 슬로다운(SS)은 \textit{Lucid에만} 작용하므로 Lucid에 한해 추가로 $\pm50\%$ 교란했다.

핵심 부호는 교란 전 구간에서 불변이다: 무튜닝 SAFA의 하위 1\% 공정성이 고정 $\alpha$를 능가한다는 관계가 유지되며(무튜닝 p1 $49.7$--$53.1$ 대 고정 $36.9$--$41.4$ --- 표는 이종, 단일 수치는 보충자료), 특히 속도계수 $\pm50\%$가 이종 무튜닝 p1을 1점 미만으로만 움직여 결론이 합성 속도 모델의 인공물이 아님을 확인했다. 이종 과부하에서 정보-우위 베이스라인의 공정성 붕괴($p1\!\approx\!0$)도 대체로 유지되나, 한 가지 예외를 정직하게 짚는다 --- Lucid는 속도계수 $-50\%$ 교란에서 p1이 0.7$\to$13.8로 출렁여, 합성 속도계수가 정보-우위 정책의 공정성 \textit{판정 자체}를 흔들 수 있음을 보인다(그래도 SAFA의 49.7과는 자릿수 차이). Lucid 전용 SS $\pm50\%$ 교란에서는 p1 변화가 1점 미만이었다.

SAFA의 잔여 내부 상수도 주 엔진 p1 기준으로 직접 흔들었다: base 1.5/3/4는 p1 50--51.5(둔감), $R$ 감쇠 0.05/0.2는 p1 98.3/67.3으로 효율--공정 균형점을 이동시킨다 --- 감쇠는 숨은 워크로드 노브가 아니라(세 트레이스에서 0.1 고정으로 일관) 균형 선호 상수임을 \S\ref{SAFA}에 명시했다.\footnote{이 균형점 이동이 \textit{매끄러운 단조 노브}는 아님을 밝혀 둔다. 구현은 $R$ 테이블의 음수 항목($R\le0$, 노드 슬롯을 크게 초과하는 부족분)을 방어적으로 0.5로 치환하는데, 기본 감쇠 0.1에서는 8-슬롯 노드의 $R\ge0.2$라 이 가드가 발동하지 않으나(따라서 주 결과는 무영향), 감쇠 $\ge0.2$에서는 부족분이 매우 큰 항이 중간 부족분보다 적합도가 높아지는 비단조가 생긴다. 따라서 0.2 수치(67.3)는 이 가드 하의 값이며, 감쇠 민감도를 단일 단조 곡선으로 외삽하지 않는다.} 요컨대 주 대비는 계수 섭동에 견고하되, Lucid 행의 절대값은 합성 계수 의존임을 함께 명시한다.
\begin{table}[htb!]
\centering
...
이를 위해 \textbf{Helios}\cite{hu2021helios}(SenseTime, SC'21) Venus 트레이스(Philly와 독립, GPU 작업 125,303개)에서 동일 스윕을 수행했다(표 \ref{tab:helios}·그림 \ref{fig:helios}, 과부하 이종 80 GPU; seed-42 50\% 서브샘플; 10정책). 방법 정정을 공개한다: Venus에는 용량(80 GPU) 초과 요청(최대 200, 1.06\%)이 있어 blocking 정책이 영구 차단되는 모집단 불일치가 있었고, 용량 초과 666건을 제거한 \textbf{동일 모집단}(62,069작업)으로 재실행했다. Philly 구도가 재현된다 --- \textbf{비차단 정보-우위 정책 전부 $p1$ 0으로 붕괴}(\texttt{lt50\%} 8.9--21.9\%)하는 반면 \textbf{무튜닝 SAFA는 $p1{=}83.9$·\texttt{lt50\%} 0.0}으로 순서 공정성을 유지한다(고정 $\alpha$ 63.7로 그다음). 효율 trade-off는 Philly보다 가파르다 --- SAFA 중앙 대기는 FIFO 대비 15\% 짧으나(37.9M 대 44.6M초) 비차단 그리디와는 자릿수가 다르며, 대형 gang(평균 6.7 GPU)이 큐를 지배해 순서 보존의 효율 비용이 크다(절대값은 80 GPU 스트레스 산물). 같은 정정을 단일 구성에 적용해도 패턴이 유지된다(SAFA $p1{=}80.0$·비차단 전부 0; 보충 \texttt{helios\_le80/}).
...
세 번째로 \textbf{Alibaba PAI GPU 트레이스}\cite{weng2022mlaas}(NSDI'22; cluster-trace-gpu-v2020)를 재생했다(표 \ref{tab:alibaba}). 완료 작업·유효 시각으로 거르고 백분율 분수-GPU를 정수로 변환한 뒤 seed-42 16.4\% 서브샘플로 120,105작업을 얻었다(부하배수 3.69$\times$/1.84$\times$/0.92$\times$로 Philly와 일치). 동일 메커니즘이 재현된다 --- 비차단 정책(LAS·Kueue·FGD·EASY)은 중앙 대기를 낮추는 대가로 7.7--15.3\%를 추월에 빠뜨리는($p1{=}0$) 반면, 무튜닝 SAFA는 \textbf{\texttt{lt50\%} 0.0\%($p1{=}98.4$)}로 추월을 제거하며 고정 $\alpha$(73.8)보다 25점 높다. 방법 정정을 여기에도 공개한다: 용량(256 GPU) 초과 26건이 blocking 정책을 차단하는 모집단 결함이 있어 $\le$256 필터 후 동일 모집단 120,079로 재실행했다(정정 전 고정 $\alpha$의 ``붕괴'' 7.9는 이 결함의 산물). 짧은-꼬리 작업이 많아 그리디의 추월이 Philly보다 오히려 크다(EASY 15.3\% 대 13.8\%).
...
SAFA는 ``어떤 작업을 먼저''만 정하고 배치는 다음 단계에 맡기므로, 그 이점이 특정 배치에 의존하지 않아야 한다. 동일 SAFA 큐 전처리를 \textbf{5개 코어 배치}(most-allocated·compact·round-robin·MCTS·단편화 인지 FGD) 위에 얹어 재실험했다(표 \ref{tab:placement}, 그림 \ref{fig:placement}). \textbf{SAFA의 $p1$이 5개 배치 전부$\times$과부하 4구성에서 50--70으로 유지}되며, 배치가 절대 큐 지연에 주는 영향은 작아(SAFA $q_{p50}$ 변동 5--15\%) 순위를 바꾸지 않는다 --- 배치 불변인 것은 SAFA 자신의 안정성이다. 특히 FGD 행(SAFA$\oplus$FGD)은 어드미션 순서와 배치 축의 \textit{결합 방향}을 검증한다: 단편화 인지 배치 위에 얹어도 p1이 50.1--61.0(다른 배치와 같은 대역)으로 유지되어 두 층위가 간섭 없이 결합됨이 확인된다(collocation 모델의 Lucid 결합은 향후 과제). 즉 SAFA의 기여는 배치 휴리스틱과 직교하며 어떤 배치 위에서도 작동한다(MCTS 비용은 most-allocated 대비 1.0--1.9$\times$로 현실적이며, most-allocated 결과는 표 \ref{tab:sim-single}의 해당 셀과 정확히 일치).
...
SAFA가 ``배치 이전의 어드미션 계층''이라는 주장을 운영 차원에서 뒷받침하기 위해, 배치 스케줄러를 교체하지 않고 SAFA를 끼워 넣는 통합 방법을 구체화한다. 쿠버네티스에서는 커스텀 스케줄러 빌드 없이 \texttt{PodSchedulingGates}로 실현한다: 각 Pod에 게이트 \texttt{safa.io/admission}를 부여해 기본 \texttt{kube-scheduler} 후보에서 잠시 빼고, SAFA 컨트롤러가 주기 $\Delta$마다 게이트된 Pod의 대기 나이·자원 요청을 관측해 식 \eqref{eq:auto-pstar}로 $P^*$를 계산한 뒤 가장 높은 Pod부터 게이트를 해제한다(Algorithm \ref{alg:k8s}). 게이트가 풀린 Pod만 기본 스케줄러가 그 순서로 binding하므로 배치 로직은 전혀 손대지 않는다 --- Kueue와 결합하면 LocalQueue 제출 직전, 커스텀 플러그인과 결합하면 QueueSort 앞의 전처리로 동작한다. 운영상 가장 중요한 점은 \textbf{게이트를 어떤 규율로 해제하느냐가 공정성을 가른다}는 것이다. 선두가 막혔을 때도 뒤 Pod을 계속 푸는 greedy 비차단 해제는 중앙 대기를 7.5배 줄이지만, 그 대가로 worst-1\% p1을 0으로 붕괴시킨다. 따라서 시뮬과 같은 p1을 지키려면 \textbf{선두가 들어갈 자리가 없을 때 그 주기의 해제를 멈춰야}(blocking-aware) 한다. 이 대비는 \S\ref{eval:b200}의 버스트 도착 B200 \textit{실측}으로 확인된다 --- blocking$+$counter가 p1 82.8인 반면 기존 greedy는 3.8이다. 다만 이 규율 하나만으로는 부족하다: blocking을 SJF·Themis 정렬에 적용해도 p1은 0이므로, p1 유지는 도착순 골격·blocking·제한적 승급이 함께 작용한 결과다. 권장 구현(blocking-aware 해제, 도착-순번 나이)은 컨트롤러 참조 구현으로 제공한다. 그 밖의 운영 요건 --- 멀티노드 gang의 노드 단위 fit 판정·잠정 점유 계상, 영구 fit 불가 작업의 wedge dead-letter 격리, 멀티테넌트에서 VC 내부 적용(VC 간 쿼타는 Kueue에 위임), Pod-수준 게이트의 etcd 부하를 피하는 Kueue suspend 결합, fail-open과 전략 내성 --- 은 \textit{멀티노드·멀티테넌트} 배포의 설계 사항이며, 본 참조 구현은 단일 노드(B200$\times$8)를 대상으로 집계 용량으로 단순화했으므로(단일 노드에서 노드 fit과 등가) 이들의 구현·검증은 향후 과제로 둔다(\S\ref{eval:threats}).
...
위 (b)의 ``나이 승급 비활성'' 한정을 \textbf{도착 버스트}로 직접 메운 추가 실측을 수행했다. 균일 도착 대신 \textbf{버스트 도착을 보존}한 트레이스(원 도착 패턴 유지, peak 12$\times$ 용량, 대기 큐 440개로 균일 대비 2.4배)에 \textbf{blocking-aware 해제 + 도착-순번(counter) 나이} 컨트롤러를 적용해 \textit{나이 승급이 활성인} 조건을 6 run(각 500잡)으로 실측했다(표 \ref{tab:b200burst}). 핵심 대비가 실측으로 재현된다: \textbf{blocking$+$counter는 $p1$ 82.8·\texttt{lt50} 0.0\%로 버스트 과부하 아사를 예방}하는 반면, 옛 거동인 greedy$+$wall은 $p1$ 3.8로 붕괴한다 --- 컨트롤러의 blocking-aware 해제가 버스트 조건에서 결정 인자다(반복 변동성: blocking $p1$ 82.8/82.4로 $\pm$0.2, FIFO 84.2/89.5). 주기 $\Delta$를 5초$\to$1초로 좁히면 dispatch가 빨라져(중앙 대기 1927$\to$443초, 할당률 59$\to$66\%) $p1$이 82.8$\to$40.1로 내려가, 운영 주기에 공정성--효율 트레이드오프가 있음을 보인다(두 $\Delta$ 모두 greedy 3.8보다 훨씬 공정). 정직하게 한정한다 --- 단일 노드·버스트에서 counter 나이는 도착 순번에 단조라 \textbf{blocking 하에서 SAFA(82.8)와 FIFO(84.2)가 사실상 동등}하다. 즉 이 실측이 입증하는 것은 ``blocking 해제가 버스트 아사를 막는다''까지이며, ``나이 승급이 단순 FIFO를 능가한다''(효율 손실 없이 아사만 제거)는 단편화가 있는 다중 노드에서만 드러나 시뮬레이션이 담당한다.
...
강건성 검증(\S\ref{eval:robust})에도 본 평가는 다음 한계를 가지므로 결과는 절대값보다 \textit{경향}으로 해석한다. (1) \textbf{실측·모델 범위}: 실 클러스터 검증은 B200$\times$8 \textit{단일 노드}·균일 도착에 한정되며(엔진 계산 충실도는 \S\ref{eval:robust:fidelity}), 다중 노드 단편화·버스트 도착의 end-to-end 실측과 반복 버스트-드레인 레짐은 향후 과제다. 네트워크·작업 간 간섭은 모델링하지 않았고(collocation 민감 정책에 영향), \textit{작업이 타입을 고정}하는 구속 환경에서는 타입-비인지 $R$이 승급 신호를 잃어 SAFA가 FIFO로 퇴화하므로 $R$의 타입별·노드묶음(멀티노드 gang)·다차원 자원 확장이 선결이다. (2) \textbf{워크로드·비교 범위}: 주 평가는 Philly 단일 트레이스 기반이며 Helios·Alibaba·사내 실측으로 보강하나 각각 유한 표본이다. 결정 변수를 고정 요청 트레이스로 정의할 수 없는 탄력 SOTA(Sia 등)는 제외했다(\S\ref{baseline-rationale}). 따라서 본 기여는 ``모든 스케줄러보다 낫다''가 아니라 ``부차 정보 없이 동작하는 배치-전 어드미션 순서 계층''으로 한정된다. (3) \textbf{지표 편향}: slowdown 계열(BSLD\cite{feitelson1998metrics})은 짧은 작업에, 도착 순서 공정성(p1)은 도착순 보존형에 구조적으로 유리할 수 있어, 독립 분배 지표(표 \ref{tab:indep})와 임계 스윕으로 교차 점검했다(\texttt{lt25/50} 대비 유지, \texttt{lt75} 일부 역전 --- \S\ref{eval:main}). 종합하면 결론은 절대 순위가 아니라 이질성·부하가 깊어질 때의 \textit{공정성 붕괴 대 유지의 질적 경향}에 무게를 둔다.

\subsection{Lifecycle overhead model}\label{eval:overhead}
이산 사건 시뮬레이터는 작업당 고정 오버헤드(스케줄링·기동·종료)를 주입하여 충실도를 높인다. 이 보정값은 가정이 아니라 \textit{실측}이다: 우리는 NVIDIA B200$\times$8 단일 노드 쿠버네티스(v1.31, kind) 클러스터에서 47개 작업을 4개 시나리오(단독 순차, 동시 8, 동시 24, 대형 gang)로 실행해 작업 수명 주기의 각 구간을 계측했다(이미지 캐시 상태 기준 --- 콜드 풀의 수십 GB CUDA 이미지는 기동을 분 단위로 늘릴 수 있어 별도 항으로 보아야 한다). 측정값에 맞춰 고정 오버헤드를 작업당 4.5--6.5초로 설정한다 --- 스케줄링 지연 0.5초, 컨테이너 기동 1.5--3.5초, 종료(teardown$\to$complete) 2.5초다. 기동 비용은 단독 배치(이미지 캐시 상태에서 p50 1초, p90 2초 $\to$ 1.5초)와 혼잡 배치(동시 2개 이상, p50 3초·p90 4초 $\to$ 3.5초)를 구분하여, 공유 노드에 여러 gang이 동시에 올라올 때의 컨테이너 초기화 경합을 반영한다.

한 가지 중요한 구분이 있다. 동시 24 시나리오에서 관측된 17--36초의 스케줄링 지연은 \textit{오버헤드가 아니라 용량 대기(큐잉)}이며, 시뮬레이터가 정책 로직으로 직접 산출하므로 고정 오버헤드에 포함하지 않는다(이중 계상 방지) --- 주입하는 것은 스케줄링 지연$+$기동$+$종료(작업당 4.5--6.5초)이며, 용량 대기(큐잉)는 정책 로직이 산출한다 --- 충실도 검증(\S\ref{eval:robust:fidelity})의 회계도 이 명세(place $=$ arrival $+$ sched\_lat $+$ startup) 기준이다.
...
고정 계수 SAFA(고정 $\alpha$)는 최적 계수를 사후(posterior) 반복 탐색으로 찾아야 했고, 해당 목적함수 기준 최적 $\alpha$가 워크로드마다 7배 달라지는 한계가 있었다. 본 연구의 핵심 진전은 이 한계를 무튜닝 SAFA로 해소한 것이다 --- 계수를 사전에 계산해 두는 대신, 관측 가능한 큐 통계(상대 나이·자원 적합도 $R$)로부터 나이 가중 계수를 무튜닝으로 적응시켜, 작업 실행 시간(duration) 추정 없이 워크로드 변화에 적응한다.

다만 SAFA는 큐 순서만을 조정하므로, 백로그가 얕은 일부 상황에서는 기아 작업을 상위로 승급시켜도 작은 작업에 자리를 추월당할 수 있다. 이는 향후 연구에서 감지(부하·나이 기반 기아 압력)와 자원 예약(EASY식 자리 잠금, 단 duration 추정 불요)을 결합하여 해소할 계획이다 --- 다만 \textit{예약}(자리 잠금)과 \textit{재정렬}(나이 승급)은 같은 큐에서 충돌할 수 있으므로, 그 상호작용을 분석해 두 메커니즘을 정합시키는 것이 결합의 선결 과제다. 또한 본 평가는 GPU 갱 트레이스(Philly·사내 K8S)에 한정되므로, 표준 워크로드 포맷(SWF) 기반 고전 병렬 작업 트레이스로 평가를 확장해 고전 HPC 배치 스케줄링 문헌과의 가교를 놓는 것도 향후 과제다.
