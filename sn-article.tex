...
\caption{most-allocated 스케줄링 후 --- 5 GPU를 요청하는 $t_2$의 HOL 블로킹으로 $t_3$--$t_5$가 함께 지연된 상태.}
\label{fig:03}
\end{figure}

\begin{figure}[htb!]
\centerline{\includegraphics[width=\columnwidth]{Pic/Fig_04.png}}
\caption{스케줄링 후 잔여 대기 큐 --- $t_0$, $t_1$이 배치된 뒤 $t_2$--$t_5$가 큐에 남아, 선두 $t_2$가 자리를 얻을 때까지 후속 작업들이 함께 적체된다.}
\label{fig:04}
\end{figure}
...
\begin{table}[htb!]
\centering
\caption{합성 계수 $\pm50\%$ 교란 하의 하위 1\% 순서 공정성 $p1$(512 이종; 그림 \ref{fig:sens-robust}). base 열은 본 절 \textit{재실행 하니스} 산출이라 주 표(표 \ref{tab:sim})와 절대값이 다를 수 있다 --- SAFA 37.4는 표 \ref{tab:ablation}의 37.3과 0.1 차이(37.35 반올림), Lucid 0.7은 주 표의 0과 갈리는데(주 sweep $q_{p50}$ 115 대 본 하니스 155) 난수 util·전역 슬로다운 상태에 의존하는 Lucid가 하니스 구성에 민감한 탓이다(SAFA·결정적 정책은 비트 일치). 따라서 이 표는 \textbf{교란 대비 상대 변화}로 읽어야 하며, 주 결과의 절대값은 표 \ref{tab:sim}을 기준으로 한다.}
\label{tab:sens}
\footnotesize
...
\begin{table}[htb!]
\centering
\caption{Helios(Venus) 과부하 이종 일반화(10정책, 동률 행 병합 --- 층위는 본문 분류 따름), 용량 초과 작업 제거 후 \textbf{동일 모집단} 62,069작업(테넌트 정보가 없는 트레이스라 Kueue는 단일-VC 비차단 FCFS로 퇴화 --- LAS·FGD와 동일 행). \texttt{lt50\%}=추월 순서 불공정 비율, $p1$=worst-1\% 순서 공정성, $q_{p50}$=중앙 대기. SAFA가 $p1{=}83.9$·\texttt{lt50\%} 0.0으로 순서 공정성 유지(비차단 정책은 전부 $p1{=}0$). 같은 동일-모집단 정정을 \textit{단일}(동종) 구성에 적용해도 패턴이 재현된다 --- 무튜닝 SAFA $p1{=}80.0$(고정 $\alpha$ 58.0)·비차단 6종 전부 $p1{=}0$(보충자료 \texttt{helios\_le80/}). 그림 \ref{fig:helios}.}
\label{tab:helios}
\footnotesize
