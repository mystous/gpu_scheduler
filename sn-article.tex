...
\begin{table*}[htb!]
\centering
\caption{관련 스케줄러의 성격별 분류와 실험 포함 여부. 본 평가는 (i) 같은 결정 변수, (ii) 같은 계층, (iii) 비교 가능한 정보·인프라 가정을 만족하는 정책만 동일 엔진 실험에 포함하고($\checkmark$), 그 밖의 최신 기법은 각 축의 대표 베이스라인으로 그 원리를 평가한다. 잡별 성능 프로파일이나 작업 내부 실행계획을 결정 변수로 삼는 탄력 SOTA(Sia·Arena·Rubick 등)는 그 결정 변수가 고정 요청 트레이스에 정의되지 않아 동일 엔진에서 제외하고 관련 연구로 다룬다(\S\ref{baseline-rationale}).}
\label{tab:related}
\footnotesize
\begin{tabular}{@{}lllcl@{}}
\toprule
...
\begin{table}[htb!]
\centering
\caption{기아 대응 정책의 보장 수준과 정보 요구 대비. EASY는 막힌 개별 작업의 완료를 보장하나 실행시간 추정을 요구하고, SAFA는 큐 정체만 절단하되 큐 관측 정보만으로 동작한다.}
\label{tab:guarantee}
\small
\begin{tabular}{@{}lll@{}}
\toprule
...
\caption{부하 의존성 종합(Philly 111k, 10정책). 윗줄 단일·아랫줄 이종, 각 열은 중앙값 q\_p50·최악 대기 q\_max·하위 1\% 순서 공정성 p1; 클러스터가 작을수록 부하가 깊다.}
\label{fig:loadcurve}
\end{figure*}
...
\caption{큐잉 지연 대 공정성 트레이드오프(Philly 111k, 10정책). 윗줄 단일·아랫줄 이종, 각 열 256/512/1024 GPU; 가로=중앙값 큐 지연(로그), 세로=p1 공정성, 좌상단일수록 빠르고 공정.}
\label{fig:tradeoff}
\end{figure*}

\begin{table*}[htb!]
\centering
\caption{단일(동종) 클러스터 결과 --- 클러스터 크기(=부하)별 (Philly 111k; 256 GPU=3.6$\times$, 512=1.8$\times$, 1024=0.9$\times$ 부하). q\_p50=중앙값 큐잉 지연(초, K=$10^3$·M=$10^6$, 낮을수록 우수), p1=하위 1\% 순서 공정성(100=완전 공정·0=완전 기아, 높을수록 우수), Alloc=평균 GPU 할당률(\%). 최악 대기 q\_max는 그림 \ref{fig:loadcurve}, makespan은 본문 참조. 부하가 깊어질수록(1024$\to$256) 정책 간 격차가 벌어진다.}
\small
...
\begin{table*}[htb!]
\centering
\caption{이종(heterogeneous) 클러스터 결과 --- 클러스터 크기(=부하)별. 열 의미는 표 \ref{tab:sim-single}과 동일(q\_max는 그림 \ref{fig:loadcurve}, makespan은 본문). 이종 고부하(256·512)에서 SJF·LAS·Kueue·EASY·Themis·FGD가 모두 p1=0(완전 기아)으로 붕괴하는 반면, SAFA는 큐 정보만으로 그 영역의 아사를 억제해 공정성을 유지한다. 저부하(1024)에서는 큐가 얕아 대부분 정책이 회복한다.}
\small
\begin{tabular}{@{}l@{\hspace{1.6em}}rrr@{\hspace{1.6em}}rrr@{\hspace{1.6em}}rrr@{}}
\toprule
 & \multicolumn{3}{c}{\textbf{q\_p50 (s)}} & \multicolumn{3}{c}{\textbf{p1}} & \multicolumn{3}{c}{\textbf{Alloc (\%)}} \\
...
\begin{table}[htb!]
\centering
\caption{고정 $\alpha$ 대비 무튜닝 SAFA ablation(6구성). 두 변형은 같은 $P^*$ 골격·단일 승급을 공유하며 계수 결정 방식만 다르다. q\_p50=중앙값 큐 지연(초, K=$10^3$·M=$10^6$, 낮을수록 우수), p1=하위 1\% 순서 공정성(높을수록 우수), Alloc=평균 GPU 할당률(\%). 여기 고정 $\alpha$는 \textit{모든 구성 공통의 단일 고정값}이며, 구성별 최적 $\alpha$와의 비교는 별도다.}
\label{tab:ablation}
\small
...
\begin{table}[htb!]
\centering
\caption{강화판 ablation --- 구성별 \textit{최적} 고정 $\alpha$(16점 그리드 $\alpha\in[0.01,2.0]$에서 p1 최대) 대 무튜닝 SAFA. 어떤 고정 $\alpha$도, 워크로드마다 최적으로 골라도, 무튜닝의 하위 1\% 순서 공정성(p1)에 도달하지 못한다(부하 5구성). q\_p50=중앙값 큐 지연(초), 괄호는 그 최적값을 낸 $\alpha$.}
...
이를 위해 \textbf{Helios}(SenseTime, SC'21)의 Venus 트레이스 --- Philly와 독립인 또 하나의 대규모 공개 트레이스(GPU 작업 125,303개로 Philly의 111,586개와 동급) --- 에서 동일 스윕을 수행했다(표 \ref{tab:helios}, 과부하 이종 80 GPU; 런타임 제약으로 seed-42 50\% 서브샘플 62,735작업, 전수 결과와 정책 순위 일치 검증, Lucid는 비용으로 제외). \textbf{본 연구의 핵심 기여 --- 부차 정보 없이 아사를 예방한다 --- 가 일반화된다}: 정보-우위 그리디 정책(LAS·Kueue·FGD)이 과부하 이종에서 작업의 20.8--21.9\%를 굶기는(\texttt{lt50\%}, 순서 공정성 점수 $<50$인 굶주린 작업 비율) 반면, 무튜닝 SAFA는 \textbf{7.3\%만 굶겨 약 1/3 수준}이며 비-FIFO 정책 중 평균 공정성도 최고다. SAFA는 고정 $\alpha$ 대비로도 이종 전 부하영역에서 일관 우위다(과부하 lt50\% 7.3 대 19.0). 다만 두 가지를 정직히 한정한다. 첫째, Venus는 멀티-GPU 작업이 훨씬 커(평균 6.7 GPU 대 Philly의 소형 위주) 과부하에서 \textit{최저 1\% 순서 공정성(worst-1\% $p1$)이 FIFO를 제외한 모든 정책에서 0으로 포화}한다 --- 즉 worst-1\% 지표 자체가 트레이스 의존적이어서, 이 영역의 변별은 더 강건한 굶주림 비율(\texttt{lt50\%})로 보고한다. 둘째, 저부하에서 동종은 전 정책이 회복($p1{\approx}100$)하나 이종은 완전 회복이 아니어서 그리디 정책은 붕괴를 남기는 가운데 SAFA가 회복을 선도한다($p1$ 70.4 대 LAS/Kueue/FGD의 0). 요컨대 Helios는 SAFA의 \textit{아사 예방}이 독립 대규모 트레이스에서 재현됨을 확인하며, 동시에 worst-1\% $p1$이라는 \textit{지표}가 워크로드에 따라 포화할 수 있음을 드러낸다.

\begin{table}[htb!]
\centering
\caption{독립 대규모 트레이스 일반화 --- \textbf{Helios}(SenseTime, SC'21) Venus(125k GPU 작업, Philly와 독립), 과부하 이종 80 GPU(3.17$\times$). \texttt{lt50\%}=순서 공정성 점수 $<50$인 굶주린 작업 비율(낮을수록 공정). 무튜닝 SAFA가 정보-우위 그리디(LAS·Kueue·FGD)의 약 1/3로 굶주림을 억제 --- 아사 예방이 일반화된다. (worst-1\% $p1$은 Venus의 대형 gang으로 비-FIFO 전 정책이 0 포화하여 본 영역의 변별은 \texttt{lt50\%}로 보고; \S\ref{eval:robust:trace}.)}
\label{tab:helios}
\footnotesize
\begin{tabular}{@{}l@{\hspace{1.2em}}rrr@{}}
\toprule
\textbf{정책} & fair\_mean & \textbf{lt50\%}$\downarrow$ & $q_{p50}$ (s) \\
\midrule
...
SAFA(고정 $\alpha$) & 78.2  & 19.0 & 1{,}782{,}168 \\
\textbf{SAFA} & \textbf{86.8} & \textbf{7.3} & 4{,}691{,}516 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htb!]
\centering
\caption{독립 트레이스 일반화 --- Philly와 분리된 \textit{사내 K8S 실측 트레이스}(2024, A100/A30, 368작업)에서의 하위 1\% 순서 공정성($p1$). 클러스터 크기로 부하를 조절했다(8 GPU$=$2.45$\times$ 과부하, 16$=$1.22$\times$, 32$=$0.61$\times$ 저부하). 과부하·임계(8·16)에서 정보-우위 베이스라인은 전부 $p1{=}0$으로 붕괴하고 FIFO·SAFA만 공정성을 지킨다 --- Philly와 동일 패턴.}
\label{tab:inhouse}
\footnotesize
...
무튜닝 SAFA는 FIFO 수준의 순서 공정성을 유지(Philly 기준 하위 1\% 50--76 수준)하면서 중앙값 대기를 FIFO 대비 50--61\% 단축하여, 공정성을 포기하지 않고 빨라졌다. 단일·저경합에서는 Lucid 같은 collocation 정책이 더 빠르고 공정할 수 있으나, 이질성·부하가 깊어질수록 다른 빠른 정책들이 아사(공정성 붕괴)를 겪는 반면 SAFA는 큐 정보만으로 그 아사를 예방했고, 독립 트레이스(Helios)에서도 굶주린 잡 비율을 정보-우위 그리디의 약 1/3로 억제했다. 즉 SAFA는 별도 프로파일이나 부차 정보 없이 큐에서 관측되는 정보만으로, 코어 스케줄러를 그대로 둔 채 그 위에서 아사를 예방하면서 공정성을 유지하는 메타 스케줄러다.
