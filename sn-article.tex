...
\begin{figure*}[htb!]
\centering
\begin{subfigure}{0.47\textwidth}\includegraphics[width=\linewidth]{Pic/Fig_01.png}\caption{스케줄링 전 서버 풀(단편화)}\label{fig:concept-a}\end{subfigure}\hfill
\begin{subfigure}{0.47\textwidth}\includegraphics[width=\linewidth]{Pic/Fig_02.png}\caption{동일 시점의 대기 큐}\label{fig:concept-b}\end{subfigure}

\medskip
\begin{subfigure}{0.47\textwidth}\includegraphics[width=\linewidth]{Pic/Fig_03.png}\caption{스케줄링 후 --- $t_2$의 HOL 블로킹}\label{fig:concept-c}\end{subfigure}\hfill
\begin{subfigure}{0.47\textwidth}\includegraphics[width=\linewidth]{Pic/Fig_04.png}\caption{잔여 대기 큐($t_2$--$t_5$ 적체)}\label{fig:concept-d}\end{subfigure}
\caption{HOL 블로킹의 발생. (a) 단편화된 서버 풀과 (b) 대기 큐에서 most-allocated가 $t_0$·$t_1$을 배치하면, (c) 5-GPU $t_2$가 빈 슬롯이 충분함에도 한 갱으로 들어갈 자리가 없어 HOL 블로킹을 일으키고, (d) 그 뒤의 $t_3$--$t_5$가 큐에 함께 적체된다.}
\label{fig:concept}
\end{figure*}
...
\begin{table}[htb!]
\centering
\caption{독립 공정성 지표 교차 확인(512 이종, SAFA가 최적화하지 않은 지표). 본문 \S\ref{eval:main}.}
\label{tab:indep}
\footnotesize
...
\caption{큐 지연 대 순서 공정성 트레이드오프(좌상단=빠르고 공정). 마커는 기준선·어드미션 순서 비교군·SAFA(범례). 본문 \S\ref{eval:main}.}
\label{fig:tradeoff}
\end{figure*}

\begin{table*}[htb!]
\centering
\caption{클러스터 결과(Philly 111k; 256/512/1024\,GPU $=$ 3.6/1.8/0.9$\times$ 부하; \textbf{위 블록}$=$단일 동종, \textbf{아래 블록}$=$이종). 지표 정의 \S\ref{eval:sim}, $q_{\max}$는 그림 \ref{fig:loadcurve}. 행 구획은 \S\ref{eval:impl}의 층위 분류.}
\small
...
\begin{table}[htb!]
\centering
\caption{성분 ablation(Philly 3구성): 단일 승급에서 $R$ 제거 시 FIFO 퇴화, 전체 정렬 aging은 $w$ 보간. 변형 $q_{p50}$의 ``$=$FIFO''는 그 구성 FIFO값. 본문 \S\ref{eval:ablation:comp}.}
...
\begin{table}[htb!]
\centering
\caption{구성별 0.01--2.0 그리드 내 최선 고정 $\alpha$ 대 무튜닝 SAFA. 본문 \S\ref{eval:ablation:grid}, 그림 \ref{fig:ablation-grid}.}
...
\begin{table}[htb!]
\centering
\caption{합성 계수 $\pm50\%$ 교란 하 $p1$(512 이종). base는 재실행 하니스 산출이라 주 표(표 \ref{tab:sim-single})와 절대값이 갈릴 수 있어(특히 난수 util에 민감한 Lucid) \textbf{교란 대비 상대 변화}로 읽는다 --- 상세 \S\ref{eval:robust:sens}. 그림 \ref{fig:sens-robust}.}
\label{tab:sens}
\footnotesize
...
\begin{table}[htb!]
\centering
\caption{Helios(Venus) 과부하 이종 일반화 --- 용량 초과 작업 제거 후 동일 모집단 62,069작업(10정책, 동률 행 병합). SAFA $p1{=}83.9$로 순서 공정성 유지(비차단 전부 0). 본문 \S\ref{eval:robust:trace}, 그림 \ref{fig:helios}.}
\label{tab:helios}
\footnotesize
...
\caption{Helios(Venus) 과부하 이종(동일 모집단): \texttt{lt50\%} 대 평균 순서 공정성. 마커는 그림 \ref{fig:tradeoff}과 동일. 본문 \S\ref{eval:robust:trace}.}
\label{fig:helios}
\end{figure}

\begin{table}[htb!]
\centering
\caption{사내 K8S 실측 트레이스(368작업) 부하점별 하위 1\% 순서 공정성 $p1$(FIFO$<$100은 동률 도착 tie-break의 소표본 효과). 본문 \S\ref{eval:robust:trace}.}
\label{tab:inhouse}
\footnotesize
...
\begin{table}[htb!]
\centering
\caption{Alibaba PAI(NSDI'22, 120k 작업) 과부하 이종(256 GPU, 3.69$\times$; 동일 모집단 120,079작업, 10정책). 무튜닝 SAFA가 \texttt{lt50\%} 0.0·$p1{=}98.4$로 추월 불공정을 제거. 본문 \S\ref{eval:robust:trace}.}
\label{tab:alibaba}
\footnotesize
...
\begin{table}[htb!]
\centering
\caption{세 독립 트레이스 종합(과부하 이종 한 점씩)의 추월 순서 불공정 \texttt{lt50\%} --- SAFA가 셋 모두 비차단 정책 중 최저. 본문 \S\ref{eval:robust:trace}.}
\label{tab:trace3}
\footnotesize
...
\begin{table}[htb!]
\centering
\caption{배치 무관성 --- 5개 코어 배치별 SAFA 하위 1\% 순서 공정성 $p1$(FGD 행 $=$ SAFA$\oplus$FGD 결합). 본문 \S\ref{eval:robust:placement}, 그림 \ref{fig:placement}.}
\label{tab:placement}
\footnotesize
...
\caption{5개 코어 배치(FGD 포함)별 SAFA 하위 1\% 순서 공정성. 본문 \S\ref{eval:robust:placement}.}
\label{fig:placement}
\end{figure}
...
\begin{table}[htb!]
\centering
\caption{B200$\times$8 버스트 도착 실측(나이 승급 활성, 6 run$\times$500잡). blocking-aware 해제가 버스트 아사를 예방($p1$ 82.8), greedy는 붕괴(3.8). 본문 \S\ref{eval:b200}.}
\label{tab:b200burst}
\footnotesize
...
\begin{table}[htb!]
\centering
\caption{B200$\times$8 단일 노드 실측(500작업, 과부하): \texttt{lt50\%}·평균 순서 공정성. 본문 \S\ref{eval:b200}.}
\label{tab:b200}
\footnotesize
\begin{tabular}{@{}l@{\hspace{1.2em}}rrrr@{}}
\toprule
...
\caption{B200$\times$8 실측 --- 중앙 대기 대 평균 순서 공정성(좌상단=빠르고 공정). 그리디(kube-sched·SJF·Themis)는 하단, 예약·admission(EASY·Kueue·LAS)과 SAFA는 상단. 본문 \S\ref{eval:b200}.}
\label{fig:b200}
\end{figure}
