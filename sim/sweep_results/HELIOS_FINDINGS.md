...
> Sia는 전 스윕에서 제외(SQUAD 비교 10정책: 9정책 + lucid). **본 실험은 확인 절차가 아니라
...
### 3.2 lucid 추가 — 10정책 완성(이전 제외분 보강)
이전 스윕에서 lucid는 본 트레이스 규모에서 과부하 구성당 25분+ 소요로 잠정 제외됐었다. 본
보강 실행에서 **lucid를 동일 6구성(80/192/448 × single/hetero, seed-42 50% 서브샘플)으로 완주**
시켰다(과부하 80 single 1,078s, 80 hetero 1,078s, 192 hetero 776s, 저부하 448 single 23s 등; 전
구성 완료, 미완 구성 없음). 따라서 본 일반화 실험의 비교군은 **10정책: fifo, sjf, las, kueue,
easy, themis, fgd, sfqa(고정 α), sfqa-auto(무튜닝 헤드라인), lucid**. Sia는 지침대로 전 스윕 제외.

> **lucid 거동 요약(데이터 중심):** lucid는 활용률 최적화(alloc 96~98%, 전 정책 최상위)와
> 짧은-잡 우선 경향을 보여 **q_p50가 매우 낮다**(과부하 hetero 67s, 중부하 hetero 2s — sjf/themis
> 급). 동시에 **q_max는 전 정책 최대급**(80 hetero 92.4M s)으로, 소수의 큰 gang 잡이 만성적으로
> 추월당한다. 그러나 **굶주림 비율 lt50%는 의외로 낮다**(80 hetero 8.5%): 그리디 정보-우위
> 정책(las/kueue/easy/fgd 20.8~21.9%)보다 굶주린 잡이 적고, sfqa-auto(7.3%) 다음 수준이다.
> fair_p1은 과부하·중부하에서 0(타 비-FIFO와 동일 포화). → **예상(그리디처럼 추월 불공정 큼)과
> 부분만 일치**: 최악 큐잉(q_max)·worst-1%는 그리디만큼 나쁘지만, 굶주림 *비율*은 그리디보다
> 낮아 fair_mean이 높다(80 hetero 91.4, 그리디 76~78). lucid의 활용률·짧은잡 우선이 다수 잡의
> 슬로다운을 억제하기 때문이다.
...
## 4. 10정책 표 (과부하 80 · 중부하 192 · 저부하 448 × single·hetero)
...
| lucid | 91.4 | 8.5 | **67** | 7.1 |

→ **과부하 hetero에서 sfqa-auto의 굶주린-잡 비율(lt50%)=7.3%로 10정책 중 최저**(fifo 제외).
las/kueue/easy/fgd(정보-우위 그리디)는 20.8~21.9%, sfqa 고정 19.0%, sjf 10.4%, themis 11.0%,
**lucid 8.5%**(sfqa-auto 다음). **그러나 fair_p1은 sfqa-auto 포함 전부 0(포화)** — "p1≥50 유지"
목표는 이 영역에서 **달성 못함**. lucid는 q_p50=67s로 최저(짧은잡 우선)이나 q_max=92.4M로 최악
(소수 큰 gang 잡 만성 추월) — *중앙은 빠르고 꼬리는 심한* 전형적 활용률-최적화 거동.
...
있는 **lt50%(굶주린 잡 비율)에서는 sfqa-auto가 7.3%로 10정책 최저**(lucid 8.5% 2위; 그리디 baseline 20.8~21.9%의
...
### 10정책으로 본 결과 (lucid 합류)

lucid를 추가해 **10정책**으로 본 결과, **헤드라인 영역인 과부하 80 hetero**의 굶주림 비율
**lt50%** 순위는 (낮을수록 공정): **sfqa-auto 7.3 < lucid 8.5 < sjf 10.4 < themis 11.0 < sfqa(고정)
19.0 < las=kueue=fgd 20.8 < easy 21.9** (fifo 0.0은 전원 대기로 무의미). 즉 **무튜닝 sfqa-auto가
여전히 1위**이며, lucid가 2위로 합류해 *정보-우위 그리디(las/kueue/easy/fgd) 4종이 하위권을
독식*하는 구도를 더 선명히 한다. q_p50(중앙 큐잉)에서는 lucid가 **67s로 10정책 최저**(sjf 530,
themis 1,948보다 빠름) — 짧은-잡 우선·활용률 최적화의 직접 결과다. 그러나 lucid의 **q_max=92.4M
s는 10정책 최대**이고 **fair_p1=0**(과부하 포화)이라, *중앙은 가장 빠르되 최악 꼬리·worst-1%는
가장 나쁜* 거동을 보인다.

→ 따라서 "lucid가 그리디처럼 추월 불공정이 큰가?"라는 가설은 **지표에 따라 갈린다(부분만 참)**:
(a) **꼬리/worst-1%(q_max·fair_p1)** 기준으로는 그렇다 — lucid의 q_max는 그리디와 동급 최악이고
worst-1% 포화도 동일. (b) 그러나 **굶주림 *비율*(lt50%)·평균 공정성(fair_mean)** 기준으로는 lucid가
그리디보다 **유의하게 더 공정**하다(lt50% 8.5 vs 20.8~21.9; fair_mean 91.4 vs 76~78). lucid는
활용률을 극대화(96~98%)하며 다수의 짧은 잡 슬로다운을 억제하므로, *소수 큰 gang 잡을 희생해
다수를 보호*하는 패턴이다. 이는 sfqa-auto가 굶주림(lt50%)과 worst-1%(중부하 fair_p1=10.2,
저부하 70.4)를 *동시에* 개선하는 것과 대비되는 lucid의 한계점이다.
...
\textbf{10 policies}. \textsc{lt50\%} = fraction of jobs whose order-fairness
score $<50$ (starved; lower is fairer). \textsc{sfqa-auto} is the tuning-free
headline.}
...
3. **lucid 합류(10정책 완성):** 이전 잠정 제외분을 보강 실행해 lucid를 동일 6구성 완주(전
   구성 완료, 미완 없음). lucid는 활용률·짧은잡 우선으로 q_p50은 최저이나 q_max·worst-1%는
   최악급(굶주림 *비율* lt50%은 그리디보다 낮음 → §5 참고). Sia만 전 스윕 제외 유지.
